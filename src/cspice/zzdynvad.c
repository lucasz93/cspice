/* zzdynvad.f -- translated by f2c (version 19991025).
...
extern zzdynvad_init_t __zzdynvad_init;
static inline zzdynvad_state_t* get_zzdynvad_state(cspice_t* state) {
	if (!state->zzdynvad)
		state->zzdynvad = __cspice_allocate_module(sizeof(
	zzdynvad_state_t), &__zzdynvad_init, sizeof(__zzdynvad_init));
	return state->zzdynvad;

}

/* $Procedure ZZDYNVAD ( Fetch array, d.p. frame kernel variable ) */
/* Subroutine */ int zzdynvad_(cspice_t* __global_state, char *frname, 
	integer *frcode, char *item, integer *maxn, integer *n, doublereal *
	values, ftnlen frname_len, ftnlen item_len)
{
    ...
    /* Module state */
    zzdynvad_state_t* __state = get_zzdynvad_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling */
    ...
    chkin_(__global_state, "ZZDYNVAD", (ftnlen)8);
...
/*     Convert the frame code to a string. */
	...
	chkout_(__global_state, "ZZDYNVAD", (ftnlen)8);
	return 0;
    }
...
/*            'FRAME_#_#' */
	...
	dtpool_(__global_state, kvname, &found, n, dtype, (ftnlen)32, (ftnlen)
		1);
    } else {
...
/*           kernel variable name. */
	    ...
	    setmsg_(__global_state, "Kernel variable # was expected to be pr"
		    ...
		    "Usually this type of problem is due to an error in a fra"
		    "me definition provided in a frame kernel.", (ftnlen)416);
...
/*     error message if need be. */
	...
	setmsg_(__global_state, "The kernel variable # has used to define fr"
		"ame # was expected to have double precision data type but in"
		" fact has  character type.  Usually this type of problem is "
		"due to an error in a frame definition provided in a frame ke"
		"rnel.", (ftnlen)228);
	errch_(__global_state, "#", kvname, (ftnlen)1, (ftnlen)32);
	errch_(__global_state, "#", frname, (ftnlen)1, frname_len);
    ...
    if (*n > *maxn) {
	setmsg_(__global_state, "The kernel variable # has used to define fr"
		"ame # was expected to have size not exceeding # but in fact "
		"has size #. Usually this type of problem is due to an error "
...
/*     Look up the kernel variable. */
    ...
    gdpool_(__global_state, kvname, &__state->c__1, maxn, n, values, &found, (
	    ftnlen)32);
    if (! found) {
	setmsg_(__global_state, "Variable # not found after DTPOOL indicated"
		" it was present in pool.", (ftnlen)67);
	errch_(__global_state, "#", kvname, (ftnlen)1, (ftnlen)32);

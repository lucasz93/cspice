...
static saelgv_state_t* get_saelgv_state() {
	...
	smajor, doublereal *sminor)
{
    ...
    /* Module state */
    saelgv_state_t* __state = get_saelgv_state();
/* $ Abstract */
...
/*     Copy the input vectors. */
    ...
    moved_(vec1, &__state->c__3, tmpvc1);
    moved_(vec2, &__state->c__3, tmpvc2);
...
/* Computing MAX */
    ...
    scale = max(d__1,d__2);
    if (scale == 0.) {
	cleard_(&__state->c__3, smajor);
	cleard_(&__state->c__3, sminor);

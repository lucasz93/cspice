/* saelgv.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int saelgv_(cspice_t* __global_state, doublereal *vec1, 
	doublereal *vec2, doublereal *smajor, doublereal *sminor)
{
    ...
    extern /* Subroutine */ int diags2_(cspice_t*, doublereal *, doublereal *,
	     doublereal *);
    doublereal tmpvc1[3];
    doublereal tmpvc2[3];
    ...
    /* Module state */
    saelgv_state_t* __state = get_saelgv_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SAELGV", (ftnlen)6);
    }
...
/*     Copy the input vectors. */
    ...
    moved_(__global_state, vec1, &__state->c__3, tmpvc1);
    moved_(__global_state, vec2, &__state->c__3, tmpvc2);
...
/* Computing MAX */
    d__1 = vnorm_(__global_state, tmpvc1), d__2 = vnorm_(__global_state, 
	    tmpvc2);
    scale = max(d__1,d__2);
    if (scale == 0.) {
	cleard_(__global_state, &__state->c__3, smajor);
	cleard_(__global_state, &__state->c__3, sminor);
	chkout_(__global_state, "SAELGV", (ftnlen)6);
	return 0;
    }
    for (i__ = 1; i__ <= 3; ++i__) {
	tmpvc1[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(&
		__global_state->f2c, "tmpvc1", i__1, "saelgv_", (ftnlen)435)] 
		= tmpvc1[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(&
		__global_state->f2c, "tmpvc1", i__2, "saelgv_", (ftnlen)435)] 
		/ scale;
	tmpvc2[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(&
		__global_state->f2c, "tmpvc2", i__1, "saelgv_", (ftnlen)436)] 
		= tmpvc2[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(&
		__global_state->f2c, "tmpvc2", i__2, "saelgv_", (ftnlen)436)] 
		/ scale;
    }

/*     Compute S and diagonalize it: */

    s[0] = vdot_(__global_state, tmpvc1, tmpvc1);
    s[1] = vdot_(__global_state, tmpvc1, tmpvc2);
    s[2] = s[1];
    s[3] = vdot_(__global_state, tmpvc2, tmpvc2);
    diags2_(__global_state, s, eigval, c__);
...
/*        The second eigenvector corresponds to the semi-major axis. */
    ...
    vlcom_(__global_state, &c__[(i__1 = (major << 1) - 2) < 4 && 0 <= i__1 ? 
	    i__1 : s_rnge(&__global_state->f2c, "c", i__1, "saelgv_", (ftnlen)
	    469)], tmpvc1, &c__[(i__2 = (major << 1) - 1) < 4 && 0 <= i__2 ? 
	    i__2 : s_rnge(&__global_state->f2c, "c", i__2, "saelgv_", (ftnlen)
	    469)], tmpvc2, smajor);
    vlcom_(__global_state, &c__[(i__1 = (minor << 1) - 2) < 4 && 0 <= i__1 ? 
	    i__1 : s_rnge(&__global_state->f2c, "c", i__1, "saelgv_", (ftnlen)
	    470)], tmpvc1, &c__[(i__2 = (minor << 1) - 1) < 4 && 0 <= i__2 ? 
	    i__2 : s_rnge(&__global_state->f2c, "c", i__2, "saelgv_", (ftnlen)
	    470)], tmpvc2, sminor);

...
/* Subroutine */ int zzekac03_(integer *handle, integer *segdsc, integer *
	coldsc, char *cvals, logical *nlflgs, integer *rcptrs, integer *
	wkindx, ftnlen cvals_len)
{
    ...
    /* Local variables */
    char page[1024];
    integer tree, from, room;
    extern /* Subroutine */ int zzektr1s_(integer *, integer *, integer *, 
	    integer *), zzekcnam_(integer *, integer *, char *, ftnlen), 
	    zzekordc_(char *, logical *, logical *, integer *, integer *, 
	    ftnlen), zzekpgwc_(integer *, integer *, char *, ftnlen), 
	    zzekspsh_(integer *, integer *), zzektrit_(integer *, integer *);
    integer i__, n, p, mbase, ndata, pbase;
    extern /* Subroutine */ int chkin_(char *, ftnlen), errch_(char *, char *,
	     ftnlen, ftnlen);
    integer class__, nnull;
    extern integer rtrim_(char *, ftnlen);
    integer p2, nrows;
    extern logical return_(void);
    char column[32];
    integer adrbuf[1014], bufptr, colidx, colwid, dscbas, idxtyp, nchars, 
	    nlinks, nulptr, nwrite, remain, strlen, to;
    logical fixlen, indexd, nullok;
    extern /* Subroutine */ int setmsg_(char *, ftnlen), errint_(char *, 
	    integer *, ftnlen), sigerr_(char *, ftnlen), chkout_(char *, 
	    ftnlen), prtenc_(integer *, char *, ftnlen), prtdec_(char *, 
	    integer *, ftnlen), dasudi_(integer *, integer *, integer *, 
	    integer *);
    integer pos;
...
/*        The link count starts out at zero. */
	...
	prtenc_(&c__0, page + 1019, (ftnlen)5);
    }
...
/*        Examine a column entry.  Write it out if it's non-null. */
	...
	if (nullok && nlflgs[from - 1]) {
	    adrbuf[(i__1 = bufptr - 1) < 1014 && 0 <= i__1 ? i__1 : s_rnge(
		    "adrbuf", i__1, "zzekac03_", (ftnlen)382)] = -2;
	} else {
...
/*           determine the length of an input string. */
	    ...
	    if (fixlen) {
/* Computing MIN */
		i__1 = rtrim_(cvals + (from - 1) * cvals_len, cvals_len);
		strlen = min(i__1,colwid);
	    } else {
		strlen = rtrim_(cvals + (from - 1) * cvals_len, cvals_len);
	    }
	    adrbuf[(i__1 = bufptr - 1) < 1014 && 0 <= i__1 ? i__1 : s_rnge(
		    "adrbuf", i__1, "zzekac03_", (ftnlen)400)] = to + pbase;
	    pos = 1;

/*           Start out with the string length. */

	    prtenc_(&strlen, page + (to - 1), (ftnlen)5);
	    n += 5;
	    to = n + 1;
	    nchars = strlen;
	    while(nchars > 0) {
...
/*                 Add a link to the current page. */
		    ...
		    prtdec_(page + 1019, &nlinks, (ftnlen)5);
		    i__1 = nlinks + 1;
		    prtenc_(&i__1, page + 1019, (ftnlen)5);
		} else {

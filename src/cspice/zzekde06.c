/* zzekde06.f -- translated by f2c (version 19991025).
...
extern zzekde06_init_t __zzekde06_init;
static inline zzekde06_state_t* get_zzekde06_state(cspice_t* state) {
	if (!state->zzekde06)
		state->zzekde06 = __cspice_allocate_module(sizeof(
	zzekde06_state_t), &__zzekde06_init, sizeof(__zzekde06_init));
	return state->zzekde06;

}

/* $Procedure      ZZEKDE06 ( EK, delete column entry, class 6 ) */
/* Subroutine */ int zzekde06_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, integer *recptr)
{
    ...
    /* Module state */
    zzekde06_state_t* __state = get_zzekde06_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "ZZEKDE06", (ftnlen)8);
...
/*     access?  Signal an error if not. */
	...
	chkout_(__global_state, "ZZEKDE06", (ftnlen)8);
	return 0;
    }
...
/*        count for the entry. */
	...
	zzekgei_(__global_state, handle, &datptr, &nelts);

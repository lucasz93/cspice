...
/* Subroutine */ int spkapo_(cspice_t* __global_state, integer *targ, 
	...
	*ptarg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkapo_state_t* __state = get_spkapo_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SPKAPO", (ftnlen)6);
    }
...
/*        Locate the flag in our list of flags. */
	...
	if (i__ == 0) {
	    ...
	    chkout_(__global_state, "SPKAPO", (ftnlen)6);
	    return 0;
	}
...
/*     light time. */
    ...
    spkgps_(__global_state, targ, et, ref, &__state->c__0, ptarg, lt, ref_len)
	    ;
...
/*     the observer remains where he is. */
	...
	d__1 = *et + ltsign * *lt;
	spkgps_(__global_state, targ, &d__1, ref, &__state->c__0, ptarg, lt, 
		ref_len);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "SPKAPO", (ftnlen)6);
	    return 0;

...
static spkapo_state_t* get_spkapo_state() {
	...
	ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkapo_state_t* __state = get_spkapo_state();
/* $ Abstract */
...
/*     light time. */
    ...
    spkgps_(targ, et, ref, &__state->c__0, ptarg, lt, ref_len);
    if (failed_()) {
	chkout_("SPKAPO", (ftnlen)6);
	return 0;
    }
...
/*     the observer remains where he is. */
	...
	d__1 = *et + ltsign * *lt;
	spkgps_(targ, &d__1, ref, &__state->c__0, ptarg, lt, ref_len);
	if (failed_()) {
	    chkout_("SPKAPO", (ftnlen)6);
	    return 0;
	}

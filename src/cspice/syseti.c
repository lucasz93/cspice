...
/* Subroutine */ int syseti_(cspice_t* __global_state, char *name__, integer *
	...
	name_len, ftnlen tabsym_len)
{
    ...
    /* Module state */
    syseti_state_t* __state = get_syseti_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SYSETI", (ftnlen)6);
    }
...
/*     to one. */
	...
	if (dimval > 1) {
	    i__1 = dimval - 1;
	    remlai_(__global_state, &i__1, &locval, &tabval[6], &nval);
...
/*     room for one extra addition in all three tables. */
	...
	setmsg_(__global_state, "SYSETI: Addition of the new symbol # causes"
...
/*     symbol dimension one, and put the value in the right place. */
    ...
    chkout_(__global_state, "SYSETI", (ftnlen)6);
    return 0;
} /* syseti_ */

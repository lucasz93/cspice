/* zzeklltd.f -- translated by f2c (version 19991025).
...
extern zzeklltd_init_t __zzeklltd_init;
static inline zzeklltd_state_t* get_zzeklltd_state(cspice_t* state) {
	if (!state->zzeklltd)
		state->zzeklltd = __cspice_allocate_module(sizeof(
	zzeklltd_state_t), &__zzeklltd_init, sizeof(__zzeklltd_init));
	return state->zzeklltd;

}

/* $Procedure ZZEKLLTD ( EK, last less than, d.p. ) */
/* Subroutine */ int zzeklltd_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, doublereal *dkey, integer *prvloc, 
	integer *prvptr)
{
    ...
    /* Module state */
    zzeklltd_state_t* __state = get_zzeklltd_state(__global_state);
/* $ Abstract */
...
/*     If the column's not indexed, we have no business being here. */
	...
	chkin_(__global_state, "ZZEKLLTD", (ftnlen)8);
	...
	chkout_(__global_state, "ZZEKLLTD", (ftnlen)8);
	return 0;
    }
...
/*        are less than the input value. */
	...
	*prvptr = 0;
	    ...
	    __state->c__0, &__state->c_false, (ftnlen)1)) {
...
/*           is halfway between BEGIN and END. */
	    ...
	    if (zzekscmp_(__global_state, &__state->c__5, handle, segdsc, 
		    coldsc, &midptr, &__state->c__1, &__state->c__2, " ", 
		    dkey, &__state->c__0, &__state->c_false, (ftnlen)1)) {

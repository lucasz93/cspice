/* zzekue02.f -- translated by f2c (version 19991025).
...
extern zzekue02_init_t __zzekue02_init;
static inline zzekue02_state_t* get_zzekue02_state(cspice_t* state) {
	if (!state->zzekue02)
		state->zzekue02 = __cspice_allocate_module(sizeof(
	zzekue02_state_t), &__zzekue02_init, sizeof(__zzekue02_init));
	return state->zzekue02;

}

/* $Procedure      ZZEKUE02 ( EK, update column entry, class 2 ) */
/* Subroutine */ int zzekue02_(cspice_t* __global_state, integer *handle, 
	...
	logical *isnull)
{
    ...
    /* Module state */
    zzekue02_state_t* __state = get_zzekue02_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "ZZEKUE02", (ftnlen)8);
...
/*     access?  Signal an error if not. */
	...
	chkout_(__global_state, "ZZEKUE02", (ftnlen)8);
	return 0;
    }
...
/*           needed, so we have one less link to this page. */
	    ...
	    zzekpgpg_(__global_state, &__state->c__2, &datptr, &p, &pbase);
...
/*           data addition routine doesn't choke. */
	    ...
	    zzekad02_(__global_state, handle, segdsc, coldsc, recptr, dval, 
		    isnull);
	}

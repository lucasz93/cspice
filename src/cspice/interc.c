/* interc.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int interc_(cspice_t* __global_state, char *a, char *b, char 
	*c__, ftnlen a_len, ftnlen b_len, ftnlen c_len)
{
    ...
    /* Module state */
    interc_state_t* __state = get_interc_state(__global_state);
/* $ Abstract */
...
/*     Set up the error processing. */
    ...
    chkin_(__global_state, "INTERC", (ftnlen)6);
...
/* Computing MAX */
	...
	chkout_(__global_state, "INTERC", (ftnlen)6);
	return 0;
    }
...
/*        as necessary. */
	...
	if (ccard < csize) {
	    if (s_cmp(&__global_state->f2c, a + (apoint + 5) * a_len, b + (
		    bpoint + 5) * b_len, a_len, b_len) == 0) {
		++ccard;

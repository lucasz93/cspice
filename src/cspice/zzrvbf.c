...
/* Subroutine */ int zzrvbf_(cspice_t* __global_state, char *buffer, integer *
	bsize, integer *linnum, integer *namlst, integer *nmpool, char *names,
	 integer *datlst, integer *dppool, doublereal *dpvals, integer *
	chpool, char *chvals, char *varnam, logical *eof, ftnlen buffer_len, 
	ftnlen names_len, ftnlen chvals_len, ftnlen varnam_len)
{
    /* Initialized data */
    ...
    integer s_cmp(f2c_state_t*, char *, char *, ftnlen, ftnlen), i_len(
	    f2c_state_t*, char *, ftnlen), s_rnge(f2c_state_t*, char *, 
    ...
    /* Module state */
    zzrvbf_state_t* __state = get_zzrvbf_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZRVBF", (ftnlen)6);
    }
...
/*     No variable yet and no parsing errors so far. */
    ...
    s_copy(&__global_state->f2c, __state->name__, " ", (ftnlen)132, (ftnlen)1)
	    ;
    s_copy(&__global_state->f2c, __state->error, " ", (ftnlen)256, (ftnlen)1);
    __state->ncomp = 0;
...
/*     we can quit. */
	...
	s_copy(&__global_state->f2c, __state->line, " ", (ftnlen)132, (ftnlen)
		1);

/*        We need to skip blank lines... */

	while(s_cmp(&__global_state->f2c, __state->line, " ", (ftnlen)132, (
		ftnlen)1) == 0) {
	    *eof = *linnum > *bsize;
	    if (*eof) {
		chkout_(__global_state, "ZZRVBF", (ftnlen)6);
		return 0;
	    }
	    s_copy(&__global_state->f2c, __state->line, buffer + (*linnum - 1)
		     * buffer_len, (ftnlen)132, buffer_len);
	    ++(*linnum);
	}
...
/*                 separators */
			...
			__state->begs[(i__1 = __state->count - 1) < 132 && 0 
				...
				"begs", i__1, "zzrvbf_", (ftnlen)544)] = 
				__state->i__;
			__state->type__[(i__1 = __state->count - 1) < 132 && 
				0 <= i__1 ? i__1 : s_rnge(&
				__global_state->f2c, "type", i__1, "zzrvbf_", 
				(ftnlen)545)] = 5;
			__state->ends[(i__1 = __state->count - 1) < 132 && 0 
				<= i__1 ? i__1 : s_rnge(&__global_state->f2c, 
				"ends", i__1, "zzrvbf_", (ftnlen)546)] = 
				__state->i__;
			if (__state->i__ > 1) {
...
/*                          the end of the last token. */
				...
				if (__state->begs[(i__1 = __state->count - 2) 
					...
					i__2, "zzrvbf_", (ftnlen)565)]) {
				    --__state->count;
				    __state->begs[(i__1 = __state->count - 1) 
					    < 132 && 0 <= i__1 ? i__1 : 
					    s_rnge(&__global_state->f2c, 
					    "begs", i__1, "zzrvbf_", (ftnlen)
					    569)] = __state->i__ - 1;
				    __state->ends[(i__1 = __state->count - 1) 
					    < 132 && 0 <= i__1 ? i__1 : 
					    s_rnge(&__global_state->f2c, 
					    "ends", i__1, "zzrvbf_", (ftnlen)
					    570)] = __state->i__;
				    __state->type__[(i__1 = __state->count - 
					    1) < 132 && 0 <= i__1 ? i__1 : 
					    s_rnge(&__global_state->f2c, 
					    "type", i__1, "zzrvbf_", (ftnlen)
					    571)] = 6;
				} else {
			...
			__state->begs[(i__1 = __state->count - 1) < 132 && 0 
				<= i__1 ? i__1 : s_rnge(&__global_state->f2c, 
				"begs", i__1, "zzrvbf_", (ftnlen)586)] = 
				__state->i__;
			__state->ends[(i__1 = __state->count - 1) < 132 && 0 
				<= i__1 ? i__1 : s_rnge(&__global_state->f2c, 
				"ends", i__1, "zzrvbf_", (ftnlen)587)] = 
				__state->i__;
			__state->type__[(i__1 = __state->count - 1) < 132 && 
				0 <= i__1 ? i__1 : s_rnge(&
				__global_state->f2c, "type", i__1, "zzrvbf_", 
			...
			__state->begs[(i__1 = __state->count - 1) < 132 && 0 
				<= i__1 ? i__1 : s_rnge(&__global_state->f2c, 
				"begs", i__1, "zzrvbf_", (ftnlen)593)] = 
				__state->i__;
			__state->ends[(i__1 = __state->count - 1) < 132 && 0 
				<= i__1 ? i__1 : s_rnge(&__global_state->f2c, 
				"ends", i__1, "zzrvbf_", (ftnlen)594)] = 
				__state->i__;
			__state->type__[(i__1 = __state->count - 1) < 132 && 
				0 <= i__1 ? i__1 : s_rnge(&
				__global_state->f2c, "type", i__1, "zzrvbf_", 
...
/*              those cases. */
		...
		if (__state->insepf || __state->inquot && __state->even) {
		    ...
		    __state->begs[(i__1 = __state->count - 1) < 132 && 0 <= 
			    i__1 ? i__1 : s_rnge(&__global_state->f2c, "begs",
			     i__1, "zzrvbf_", (ftnlen)648)] = __state->i__;
		    __state->type__[(i__1 = __state->count - 1) < 132 && 0 <= 
			    i__1 ? i__1 : s_rnge(&__global_state->f2c, "type",
			     i__1, "zzrvbf_", (ftnlen)649)] = 2;
		}
		__state->ends[(i__1 = __state->count - 1) < 132 && 0 <= i__1 ?
			 i__1 : s_rnge(&__global_state->f2c, "ends", i__1, 
			"zzrvbf_", (ftnlen)652)] = __state->i__;
	    }
	}
...
/*           There must be at least 3 contributing tokens on this line. */
		...
		setmsg_(__global_state, "A kernel variable was not properly "
			"formed on line # text buffer.Such an assignment shou"
			"ld have the form: '<variable name> [+]= <values>'. T"
			"his line was '#'. ", (ftnlen)157);
		__state->r2 = rtrim_(__global_state, __state->line, (ftnlen)
			132);
		errint_(__global_state, "#", linnum, (ftnlen)1);
...
/*              name.  This isn't allowed. */
		...
		setmsg_(__global_state, "There is a non-printing character e"
			"mbedded in line # of the text buffer.  Non-printing "
			"characters are not allowed in kernel variable assign"
			"ments.  The non-printing character has ASCII code #. "
			, (ftnlen)192);
		errint_(__global_state, "#", linnum, (ftnlen)1);
...
/*           if longer than MAXLEN. */
		...
		setmsg_(__global_state, "A kernel pool variable name in the "
			"input buffer exceeds the maximum allowed length #1. "
			"The actual length of the variable name is #2, the of"
			"fending variable name to #3 characters: '#4'.", (
			ftnlen)184);
...
/*           They will be replaced later. */
		...
		setmsg_(__global_state, "A kernel variable was not properly "
			"formed on line # of the text buffer. Such an assignm"
			"ent should have the form: '<variable name> [+]= <val"
			"ues>'.  More specifically, the assignment operator d"
			"id not have one of the expected forms: '=' or '+='. "
			"The line was '#'. ", (ftnlen)261);
		__state->r2 = rtrim_(__global_state, __state->line, (ftnlen)
			132);
		errint_(__global_state, "#", linnum, (ftnlen)1);
		errch_(__global_state, "#", __state->line, (ftnlen)1, 
...
/*        Dates begin with @; anything else is presumed to be a number. */
	...
	while(__state->type__[(i__1 = __state->nxttok - 1) < 132 && 0 <= i__1 
		? i__1 : s_rnge(&__global_state->f2c, "type", i__1, "zzrvbf_",
		 (ftnlen)941)] != 4 && __state->nxttok <= __state->count) {
...
/*                 First perform the clean up function. */
		    ...
		    setmsg_(__global_state, "The kernel variable # has been "
			    ...
			    "ble on line # of the text buffer is not a string"
			    " value. ", (ftnlen)183);
...
/*                 First perform the clean up function. */
		    ...
		    setmsg_(__global_state, "There is a quoted string with n"
			    "o characters on line # of the text buffer. ", (
			    ftnlen)74);
		    errint_(__global_state, "#", linnum, (ftnlen)1);
		    sigerr_(__global_state, "SPICE(TYPEMISMATCH)", (ftnlen)19)
			    ;
...
/*              item. First make sure there is room to do so. */
		...
		if (__state->free <= 0) {
		    setmsg_(__global_state, "There is no room available for "
			    ...
			    "ine # of the text buffer. ", (ftnlen)153);
		    errint_(__global_state, "#", linnum, (ftnlen)1);
...
/*                 First perform the clean up function. */
		    ...
		    setmsg_(__global_state, "The kernel variable # has been "
			    ...
			    "his variable on line # of the kernel buffer is n"
			    "ot a numeric or time value. ", (ftnlen)203);
		    errch_(__global_state, "#", varnam, (ftnlen)1, 
			    __state->r1);
		    errint_(__global_state, "#", linnum, (ftnlen)1);
		    sigerr_(__global_state, "SPICE(TYPEMISMATCH)", (ftnlen)19)
...
/*                    First perform the clean up function. */
			...
			setmsg_(__global_state, "At character # of  line # i"
				"n the text buffer the character '@' appears."
				"  This character is reserved for identifying"
				" time values in assignments to kernel pool v"
				"ariables.  However it is not being used in t"
				"his fashion for the variable '#'. ", (ftnlen)
				237);
...
/*     happen if there is a vector input of the form NAME = ( ) */
	...
	setmsg_(__global_state, "The first item following the assignment ope"
		...
		"n line # of the text buffer. ", (ftnlen)192);
	i__1 = *linnum - 1;
	errint_(__global_state, "#", &i__1, (ftnlen)1);
	sigerr_(__global_state, "SPICE(BADVARASSIGN)", (ftnlen)19);
	chkout_(__global_state, "ZZRVBF", (ftnlen)6);

/* zzspklt0.f -- translated by f2c (version 19991025).
...
extern zzspklt0_init_t __zzspklt0_init;
static inline zzspklt0_state_t* get_zzspklt0_state(cspice_t* state) {
	if (!state->zzspklt0)
		state->zzspklt0 = __cspice_allocate_module(sizeof(
	zzspklt0_state_t), &__zzspklt0_init, sizeof(__zzspklt0_init));
	return state->zzspklt0;

}

/* $Procedure ZZSPKLT0 ( S/P Kernel, light time corrected state ) */
/* Subroutine */ int zzspklt0_(cspice_t* __global_state, integer *targ, 
	...
	ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspklt0_state_t* __state = get_zzspklt0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKLT0", (ftnlen)8);
    }
...
/*        had on the previous call, if any.  Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKLT0", (ftnlen)8);
	    return 0;
	}
...
/*     one-way light time. */
    ...
    zzspkgo0_(__global_state, targ, et, ref, &__state->c__0, ssbtrg, &ssblt, 
	    ref_len);
    if (failed_(__global_state)) {
	chkout_(__global_state, "ZZSPKLT0", (ftnlen)8);
	return 0;
...
/*        during the previous loop iteration. */
	...
	epoch = *et + ltsign * *lt;
	zzspkgo0_(__global_state, targ, &epoch, ref, &__state->c__0, ssbtrg, &
		ssblt, ref_len);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKLT0", (ftnlen)8);
	    return 0;

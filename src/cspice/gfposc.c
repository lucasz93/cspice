...
/* Subroutine */ int gfposc_(cspice_t* __global_state, char *target, char *
	frame, char *abcorr, char *obsrvr, char *crdsys, char *coord, char *
	relate, doublereal *refval, doublereal *adjust, doublereal *step, 
	doublereal *cnfine, integer *mw, integer *nw, doublereal *work, 
	doublereal *result, ftnlen target_len, ftnlen frame_len, ftnlen 
	abcorr_len, ftnlen obsrvr_len, ftnlen crdsys_len, ftnlen coord_len, 
	ftnlen relate_len)
{
    /* Initialized data */
    ...
    /* Module state */
    gfposc_state_t* __state = get_gfposc_state(__global_state);
/* $ Abstract */
...
/*     Check into the error subsystem. */
    ...
    chkin_(__global_state, "GFPOSC", (ftnlen)6);
...
/*     Confirm minimum window sizes. */
	...
	chkout_(__global_state, "GFPOSC", (ftnlen)6);
	return 0;
    }
...
/*     coordinate search. */
    ...
    s_copy(&__global_state->f2c, qpnams + 400, "REFERENCE FRAME", (ftnlen)80, 
	    (ftnlen)15);
    s_copy(&__global_state->f2c, qcpars + 400, frame, (ftnlen)80, frame_len);
    s_copy(&__global_state->f2c, qpnams + 480, "VECTOR DEFINITION", (ftnlen)
	    80, (ftnlen)17);
    s_copy(&__global_state->f2c, qcpars + 480, "POSITION", (ftnlen)80, (
	    ftnlen)8);
    s_copy(&__global_state->f2c, qpnams + 560, "METHOD", (ftnlen)80, (ftnlen)
	    6);
    s_copy(&__global_state->f2c, qcpars + 560, " ", (ftnlen)80, (ftnlen)1);

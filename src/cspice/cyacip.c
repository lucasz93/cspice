/* cyacip.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int cyacip_(cspice_t* __global_state, integer *nelt, char *
	dir, integer *ncycle, char *array, ftnlen dir_len, ftnlen array_len)
{
    ...
    /* Module state */
    cyacip_state_t* __state = get_cyacip_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "CYACIP", (ftnlen)6);
    }

/*     Don't even screw around if there are no elements in the array. */

    if (*nelt < 1) {
	chkout_(__global_state, "CYACIP", (ftnlen)6);
	return 0;
    }
...
/*     happen to be shorter than those in the input array. */
    ...
    outlen = i_len(&__global_state->f2c, array, array_len);

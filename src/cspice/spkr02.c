/* spkr02.f -- translated by f2c (version 19991025).
...
extern spkr02_init_t __spkr02_init;
static inline spkr02_state_t* get_spkr02_state(cspice_t* state) {
	if (!state->spkr02)
		state->spkr02 = __cspice_allocate_module(sizeof(
	spkr02_state_t), &__spkr02_init, sizeof(__spkr02_init));
	return state->spkr02;

}

/* $Procedure  SPKR02 ( SPK, read record from segment, type 2 ) */
/* Subroutine */ int spkr02_(cspice_t* __global_state, integer *handle, 
	doublereal *descr, doublereal *et, doublereal *record)
{
    ...
    /* Module state */
    spkr02_state_t* __state = get_spkr02_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SPKR02", (ftnlen)6);
    }
...
/*     Along with the record, return the size of the record. */
    ...
    dafgda_(__global_state, handle, &recadr, &i__1, &record[1]);
    chkout_(__global_state, "SPKR02", (ftnlen)6);
    return 0;
} /* spkr02_ */

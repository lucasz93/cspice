/* zzekiic1.f -- translated by f2c (version 19991025).
...
typedef int zzekiic1_state_t;
static inline zzekiic1_state_t* get_zzekiic1_state(cspice_t* state) {
	return 0;
}

/* $Procedure ZZEKIIC1 ( EK, insert into index, character, type 1 ) */
/* Subroutine */ int zzekiic1_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, char *ckey, integer *recptr, 
	logical *null, ftnlen ckey_len)
{
    ...
    /* Module state */
    zzekiic1_state_t* __state = get_zzekiic1_state(__global_state);
/* $ Abstract */
...
/*     If the column's not indexed, we have no business being here. */
	...
	chkin_(__global_state, "ZZEKIIC1", (ftnlen)8);
	...
	chkout_(__global_state, "ZZEKIIC1", (ftnlen)8);
	return 0;
    }
...
/*        Locate the predecessor of the input key, record pair. */
	...
	zzeklerc_(__global_state, handle, segdsc, coldsc, ckey, recptr, null, 
		&prvidx, &prvptr, ckey_len);
...
/*        Insert the new record number right after the item we've found. */
	...
	zzekcnam_(__global_state, handle, segdsc, column, (ftnlen)32);
	chkin_(__global_state, "ZZEKIIC1", (ftnlen)8);

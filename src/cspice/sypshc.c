/* sypshc.f -- translated by f2c (version 19991025).
...
extern sypshc_init_t __sypshc_init;
static inline sypshc_state_t* get_sypshc_state(cspice_t* state) {
	if (!state->sypshc)
		state->sypshc = __cspice_allocate_module(sizeof(
	sypshc_state_t), &__sypshc_init, sizeof(__sypshc_init));
	return state->sypshc;

}

/* $Procedure      SYPSHC ( Push a value onto a particular symbol ) */
/* Subroutine */ int sypshc_(cspice_t* __global_state, char *name__, char *
	...
	ftnlen value_len, ftnlen tabsym_len, ftnlen tabval_len)
{
    ...
    /* Module state */
    sypshc_state_t* __state = get_sypshc_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SYPSHC", (ftnlen)6);
    }
...
/*     have enough room for one extra addition in the value table. */
	...
	setmsg_(__global_state, "SYPSHC: The addition of the value $ to the "
...
/*     to the dimension, and put the value in the right place. */
    ...
    chkout_(__global_state, "SYPSHC", (ftnlen)6);
    return 0;
} /* sypshc_ */

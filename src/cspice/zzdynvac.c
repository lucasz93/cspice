/* zzdynvac.f -- translated by f2c (version 19991025).
...
static zzdynvac_state_t* get_zzdynvac_state(cspice_t* state) {
	...
	return state->zzdynvac;

}

/* $Procedure ZZDYNVAC ( Fetch array, character frame kernel variable ) */
/* Subroutine */ int zzdynvac_(cspice_t* __global_state, char *frname, 
	integer *frcode, char *item, integer *maxn, integer *n, char *values, 
	ftnlen frname_len, ftnlen item_len, ftnlen values_len)
{
    ...
    /* Module state */
    zzdynvac_state_t* __state = get_zzdynvac_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling */
    ...
    chkin_(__global_state, "ZZDYNVAC", (ftnlen)8);
...
/*     Convert the frame code to a string. */
	...
	chkout_(__global_state, "ZZDYNVAC", (ftnlen)8);
	return 0;
    }

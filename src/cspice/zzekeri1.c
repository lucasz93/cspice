/* zzekeri1.f -- translated by f2c (version 19991025).
...
extern zzekeri1_init_t __zzekeri1_init;
static inline zzekeri1_state_t* get_zzekeri1_state(cspice_t* state) {
	if (!state->zzekeri1)
		state->zzekeri1 = __cspice_allocate_module(sizeof(
	zzekeri1_state_t), &__zzekeri1_init, sizeof(__zzekeri1_init));
	return state->zzekeri1;

}

/* $Procedure ZZEKERI1 ( EK, LLE using record pointers, integer, type 1 ) */
/* Subroutine */ int zzekeri1_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, integer *ikey, integer *recptr, 
	logical *null, integer *prvidx, integer *prvptr)
{
    ...
    /* Module state */
    zzekeri1_state_t* __state = get_zzekeri1_state(__global_state);
/* $ Abstract */
...
/*     up any entries that don't exist! */
    ...
    nrec = segdsc[5];
    if (tsize > nrec) {
	chkin_(__global_state, "ZZEKERI1", (ftnlen)8);
	...
	chkout_(__global_state, "ZZEKERI1", (ftnlen)8);
	return 0;
    }
...
/*     Compare the input value to the smallest value in the column. */
    ...
    if (zzekscmp_(__global_state, &__state->c__3, handle, segdsc, coldsc, &
	    begptr, &__state->c__1, &__state->c__3, " ", &__state->c_b12, 
	    ikey, null, (ftnlen)1)) {
...
/*        input value. */
	...
	*prvptr = 0;
	return 0;
	    ...
	    coldsc, &begptr, &__state->c__1, &__state->c__3, " ", &
	    __state->c_b12, ikey, null, (ftnlen)1) && *recptr < begptr) {
...
/*     Compare the input value to the greatest value in the column. */
    ...
    if (zzekscmp_(__global_state, &__state->c__5, handle, segdsc, coldsc, &
	    endptr, &__state->c__1, &__state->c__3, " ", &__state->c_b12, 
	    ikey, null, (ftnlen)1)) {
...
/*        input value. */
	...
	*prvidx = tsize;
	...
	return 0;
	    ...
	    coldsc, &endptr, &__state->c__1, &__state->c__3, " ", &
	    __state->c_b12, ikey, null, (ftnlen)1) && endptr <= *recptr) {
...
/*        entry at record MIDPTR. */
	...
	if (zzekscmp_(__global_state, &__state->c__5, handle, segdsc, coldsc, 
		&midptr, &__state->c__1, &__state->c__3, " ", &__state->c_b12,
		 ikey, null, (ftnlen)1)) {
...
/*           IKEY, based on data values. */
		...
		coldsc, &midptr, &__state->c__1, &__state->c__3, " ", &
		__state->c_b12, ikey, null, (ftnlen)1)) {

/* spkw20.f -- translated by f2c (version 19991025).
...
extern spkw20_init_t __spkw20_init;
static inline spkw20_state_t* get_spkw20_state(cspice_t* state) {
	if (!state->spkw20)
		state->spkw20 = __cspice_allocate_module(sizeof(
	spkw20_state_t), &__spkw20_init, sizeof(__spkw20_init));
	return state->spkw20;

}

/* $Procedure SPKW20 ( SPK, write segment, type 20 ) */
/* Subroutine */ int spkw20_(cspice_t* __global_state, integer *handle, 
	integer *body, integer *center, char *frame, doublereal *first, 
	doublereal *last, char *segid, doublereal *intlen, integer *n, 
	integer *polydg, doublereal *cdata, doublereal *dscale, doublereal *
	tscale, doublereal *initjd, doublereal *initfr, ftnlen frame_len, 
	ftnlen segid_len)
{
    ...
    /* Module state */
    spkw20_state_t* __state = get_spkw20_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "SPKW20", (ftnlen)6);
...
/*     The number of sets of coefficients must be positive. */
	...
	chkout_(__global_state, "SPKW20", (ftnlen)6);
	return 0;
    }
...
/*     time. */
    ...
    if (*first > *last) {
	setmsg_(__global_state, "The segment start time: # (# TDB) is greate"
		"r than the segment end time: (# TDB).", (ftnlen)80);
...
/*     The distance and time scales must be positive. */
    ...
    if (*dscale <= 0.) {
	setmsg_(__global_state, "The distance scale is not positive.DSCALE ="
		" #", (ftnlen)45);
	errdp_(__global_state, "#", dscale, (ftnlen)1);
...
/*     Store the distance and time scales. */
    ...
    dafada_(__global_state, dscale, &__state->c__1);

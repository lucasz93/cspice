...
/* Subroutine */ int gfpa_(cspice_t* __global_state, char *target, char *
	illmn, char *abcorr, char *obsrvr, char *relate, doublereal *refval, 
	...
	relate_len)
{
    ...
    /* Module state */
    gfpa_state_t* __state = get_gfpa_state(__global_state);
/* $ Abstract */
...
/*     Check into the error subsystem. */
    ...
    chkin_(__global_state, "GFPA", (ftnlen)4);
...
/*     Confirm minimum window sizes. */
	...
	chkout_(__global_state, "GFPA", (ftnlen)4);
	return 0;
    }
    if (*nw < 5) {
...
/*     Set up a call to GFEVNT specific to the phase angle search. */
    ...
    s_copy(&__global_state->f2c, qcpars + 160, abcorr, (ftnlen)80, abcorr_len)
	    ;
    s_copy(&__global_state->f2c, qpnams + 240, "ILLUM", (ftnlen)80, (ftnlen)5)
	    ;
    s_copy(&__global_state->f2c, qcpars + 240, illmn, (ftnlen)80, illmn_len);
...
/*     Progress report and interrupt options are set to .FALSE. */
    ...
    gfevnt_(__global_state, (U_fp)gfstep_, (U_fp)gfrefn_, "PHASE ANGLE", &
	    __state->c__4, qpnams, qcpars, qdpars, qipars, qlpars, relate, 
	    refval, &tol, adjust, cnfine, &__state->c_false, (U_fp)gfrepi_, (
	    U_fp)gfrepu_, (U_fp)gfrepf_, mw, &__state->c__5, work, &
	    __state->c_false, (L_fp)gfbail_, result, (ftnlen)11, (ftnlen)80, (

/* syseld.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int syseld_(cspice_t* __global_state, char *name__, integer *
	begin, integer *end, char *tabsym, integer *tabptr, doublereal *
	tabval, doublereal *values, logical *found, ftnlen name_len, ftnlen 
	tabsym_len)
{
    ...
    /* Module state */
    syseld_state_t* __state = get_syseld_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SYSELD", (ftnlen)6);
    }
...
/*        Does this request even make sense? */
	...
	if (*begin >= 1 && *begin <= n && *end >= 1 && *end <= n && *begin <= 
	    ...
	    *found = TRUE_;
	    ...
	    i__1 = *end - *begin + 1;
	    moved_(__global_state, &tabval[locval + *begin + 4], &i__1, 
		    values);
	} else {
	    *found = FALSE_;
	}
    }
    chkout_(__global_state, "SYSELD", (ftnlen)6);
    return 0;
} /* syseld_ */

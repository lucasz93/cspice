...
static zzspklt1_state_t* get_zzspklt1_state() {
	...
	doublereal *dlt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspklt1_state_t* __state = get_zzspklt1_state();
/* $ Abstract */
...
/*     one-way light time. */
    ...
    zzspkgo1_(targ, et, ref, &__state->c__0, ssbtrg, &ssblt, ref_len);
    if (failed_()) {
	chkout_("ZZSPKLT1", (ftnlen)8);
	return 0;
    }
...
/*        during the previous loop iteration. */
	...
	epoch = *et + ltsign * *lt;
	zzspkgo1_(targ, &epoch, ref, &__state->c__0, ssbtrg, &ssblt, ref_len);
	if (failed_()) {
	    chkout_("ZZSPKLT1", (ftnlen)8);
	    return 0;
	}

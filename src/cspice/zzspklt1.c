...
extern zzspklt1_init_t __zzspklt1_init;
static inline zzspklt1_state_t* get_zzspklt1_state(cspice_t* state) {
	if (!state->zzspklt1)
		state->zzspklt1 = __cspice_allocate_module(sizeof(
	zzspklt1_state_t), &__zzspklt1_init, sizeof(__zzspklt1_init));
	return state->zzspklt1;

}

/* $Procedure ZZSPKLT1 ( S/P Kernel, light time corrected state ) */
/* Subroutine */ int zzspklt1_(cspice_t* __global_state, integer *targ, 
	...
	ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspklt1_state_t* __state = get_zzspklt1_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKLT1", (ftnlen)8);
    }
...
/*        had on the previous call, if any.  Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKLT1", (ftnlen)8);
	    return 0;
	}
...
/*     one-way light time. */
    ...
    zzspkgo1_(__global_state, targ, et, ref, &__state->c__0, ssbtrg, &ssblt, 
	    ref_len);
    if (failed_(__global_state)) {
	chkout_(__global_state, "ZZSPKLT1", (ftnlen)8);
	return 0;
...
/*        during the previous loop iteration. */
	...
	epoch = *et + ltsign * *lt;
	zzspkgo1_(__global_state, targ, &epoch, ref, &__state->c__0, ssbtrg, &
		ssblt, ref_len);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKLT1", (ftnlen)8);
	    return 0;

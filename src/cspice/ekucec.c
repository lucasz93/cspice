/* ekucec.f -- translated by f2c (version 19991025).
...
typedef int ekucec_state_t;
static inline ekucec_state_t* get_ekucec_state(cspice_t* state) {
	return 0;
}

/* $Procedure     EKUCEC ( EK, update character column entry ) */
/* Subroutine */ int ekucec_(cspice_t* __global_state, integer *handle, 
	...
	cvals, logical *isnull, ftnlen column_len, ftnlen cvals_len)
{
    ...
    extern /* Subroutine */ int zzekue03_(cspice_t*, integer *, integer *, 
	    integer *, integer *, char *, logical *, ftnlen);
    extern /* Subroutine */ int zzekue06_(cspice_t*, integer *, integer *, 
	    integer *, integer *, integer *, char *, logical *, ftnlen);


    /* Module state */
    ekucec_state_t* __state = get_ekucec_state(__global_state);
/* $ Abstract */
...
/*     This column had better be of character type. */
    ...
    dtype = coldsc[1];
    if (dtype != 1) {
	chkin_(__global_state, "EKUCEC", (ftnlen)6);
	setmsg_(__global_state, "Column # is of type #; EKUCEC only works wi"
	...
	chkout_(__global_state, "EKUCEC", (ftnlen)6);
	return 0;
    }
...
/*     handle this task. */
    ...
    if (isshad) {
	zzekrbck_(__global_state, "UPDATE", handle, segdsc, coldsc, recno, (
		ftnlen)6);
    }
...
/*        Class 3 columns contain scalar character data. */
	...
	zzekue03_(__global_state, handle, segdsc, coldsc, &recptr, cvals, 
		isnull, cvals_len);
    } else if (class__ == 6) {

/*        Class 6 columns contain array-valued character data. */

	zzekue06_(__global_state, handle, segdsc, coldsc, &recptr, nvals, 

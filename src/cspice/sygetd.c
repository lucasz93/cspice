...
/* Subroutine */ int sygetd_(cspice_t* __global_state, char *name__, char *
	tabsym, integer *tabptr, doublereal *tabval, integer *n, doublereal *
	values, logical *found, ftnlen name_len, ftnlen tabsym_len)
{
    ...
    /* Module state */
    sygetd_state_t* __state = get_sygetd_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SYGETD", (ftnlen)6);
    }
...
/*     We trust that the user has supplied enough room. */
	...
	*n = tabptr[locsym + 5];
	moved_(__global_state, &tabval[locval + 5], n, values);
    }
    chkout_(__global_state, "SYGETD", (ftnlen)6);
    return 0;
} /* sygetd_ */

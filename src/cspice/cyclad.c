...
/* Subroutine */ int cyclad_(cspice_t* __global_state, doublereal *array, 
	integer *nelt, char *dir, integer *ncycle, doublereal *out, ftnlen 
	dir_len)
{
    ...
    /* Module state */
    cyclad_state_t* __state = get_cyclad_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "CYCLAD", (ftnlen)6);
    }

/*     Don't even screw around if there are no elements in the array. */

    if (*nelt < 1) {
	chkout_(__global_state, "CYCLAD", (ftnlen)6);
	return 0;
    }
...
/*     minimum forward right cycle that is equivalent to the inputs. */
	...
	moved_(__global_state, array, nelt, out);
	chkout_(__global_state, "CYCLAD", (ftnlen)6);
	return 0;
    }

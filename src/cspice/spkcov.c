...
static inline spkcov_state_t* get_spkcov_state(cspice_t* state) {
	if (!state->spkcov)
...
/* Subroutine */ int spkcov_(cspice_t* __global_state, char *spk, integer *
	idcode, doublereal *cover, ftnlen spk_len)
{
    /* Builtin functions */
    ...
    /* Module state */
    spkcov_state_t* __state = get_spkcov_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "SPKCOV", (ftnlen)6);

/*     See whether GETFAT thinks we've got a binary SPK file. */
/*     If not, indicate the specific problem. */

    getfat_(__global_state, spk, arch, kertyp, spk_len, (ftnlen)80, (ftnlen)
	    80);
    if (s_cmp(&__global_state->f2c, arch, "XFR", (ftnlen)80, (ftnlen)3) == 0) 
	    {
	setmsg_(__global_state, "Input file # has architecture #. The file m"
		"ust be a binary SPK file to be readable by this routine.  If"
		" the input file is an SPK file in transfer format, run TOBIN"
		" on the file to convert it to binary format.", (ftnlen)207);
	errch_(__global_state, "#", spk, (ftnlen)1, spk_len);
	errch_(__global_state, "#", arch, (ftnlen)1, (ftnlen)80);
	sigerr_(__global_state, "SPICE(INVALIDFORMAT)", (ftnlen)20);
	chkout_(__global_state, "SPKCOV", (ftnlen)6);
	return 0;
    } else if (s_cmp(&__global_state->f2c, arch, "DAF", (ftnlen)80, (ftnlen)3)
	     != 0) {
	setmsg_(__global_state, "Input file # has architecture #. The file m"
		"ust be a binary SPK file to be readable by this routine.  Bi"
		"nary SPK files have DAF architecture.  If you expected the f"
		"ile to be a binary SPK file, the problem may be due to the f"
	...
	chkout_(__global_state, "SPKCOV", (ftnlen)6);
	return 0;
    } else if (s_cmp(&__global_state->f2c, kertyp, "SPK", (ftnlen)80, (ftnlen)
	    3) != 0) {
	setmsg_(__global_state, "Input file # has file type #. The file must"
		" be a binary SPK file to be readable by this routine. If you"
		" expected the file to be a binary SPK file, the problem may "
...
/*     Open the file for reading. */
    ...
    dafopr_(__global_state, spk, &handle, spk_len);

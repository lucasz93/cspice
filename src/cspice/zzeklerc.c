...
static inline zzeklerc_state_t* get_zzeklerc_state(cspice_t* state) {
	return 0;
}

/* $Procedure ZZEKLERC  ( EK, LLE, using record pointers, character  ) */
/* Subroutine */ int zzeklerc_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, char *ckey, integer *recptr, 
	logical *null, integer *prvidx, integer *prvptr, ftnlen ckey_len)
{
    extern /* Subroutine */ int zzekerc1_(cspice_t*, integer *, integer *, 
    ...
    /* Module state */
    zzeklerc_state_t* __state = get_zzeklerc_state(__global_state);
/* $ Abstract */
...
/*     If the column's not indexed, we have no business being here. */
	...
	chkin_(__global_state, "ZZEKLERC ", (ftnlen)9);
	...
	chkout_(__global_state, "ZZEKLERC ", (ftnlen)9);
	return 0;
    }
...
/*     column's index type. */
    ...
    itype = coldsc[5];
    if (itype == 1) {
	zzekerc1_(__global_state, handle, segdsc, coldsc, ckey, recptr, null, 

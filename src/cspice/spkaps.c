/* spkaps.f -- translated by f2c (version 19991025).
...
extern spkaps_init_t __spkaps_init;
static inline spkaps_state_t* get_spkaps_state(cspice_t* state) {
	if (!state->spkaps)
		state->spkaps = __cspice_allocate_module(sizeof(
	spkaps_state_t), &__spkaps_init, sizeof(__spkaps_init));
	return state->spkaps;

}

/* $Procedure SPKAPS ( SPK, apparent state ) */
/* Subroutine */ int spkaps_(cspice_t* __global_state, integer *targ, 
	...
	dlt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkaps_state_t* __state = get_spkaps_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "SPKAPS", (ftnlen)6);
...
/*        had on the previous call, if any.  Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "SPKAPS", (ftnlen)6);
	    return 0;
	}
...
/*     optionally corrected for light time. */
    ...
    spkltc_(__global_state, targ, et, ref, abcorr, stobs, starg, lt, dlt, 
	    ref_len, abcorr_len);
    if (failed_(__global_state)) {
	chkout_(__global_state, "SPKAPS", (ftnlen)6);
	return 0;

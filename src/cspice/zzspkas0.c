...
static inline zzspkas0_state_t* get_zzspkas0_state(cspice_t* state) {
	...
	return state->zzspkas0;

}

/* $Procedure ZZSPKAS0 ( SPK, apparent state ) */
/* Subroutine */ int zzspkas0_(cspice_t* __global_state, integer *targ, 
	...
	dlt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspkas0_state_t* __state = get_zzspkas0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "ZZSPKAS0", (ftnlen)8);
...
/*        had on the previous call, if any.  Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKAS0", (ftnlen)8);
	    return 0;
	}
...
/*     optionally corrected for light time. */
    ...
    zzspklt0_(__global_state, targ, et, ref, abcorr, stobs, starg, lt, dlt, 
	    ref_len, abcorr_len);
    if (failed_(__global_state)) {
	chkout_(__global_state, "ZZSPKAS0", (ftnlen)8);
	return 0;

...
/* Subroutine */ int gfrfov_(cspice_t* __global_state, char *inst, doublereal 
	*raydir, char *rframe, char *abcorr, char *obsrvr, doublereal *step, 
	doublereal *cnfine, doublereal *result, ftnlen inst_len, ftnlen 
	rframe_len, ftnlen abcorr_len, ftnlen obsrvr_len)
{
    ...
    /* Module state */
    gfrfov_state_t* __state = get_gfrfov_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "GFRFOV", (ftnlen)6);
...
/*     Check the result window's size. */
	...
	chkout_(__global_state, "GFRFOV", (ftnlen)6);
	return 0;
    }
...
/*     Look for solutions. */
    ...
    gffove_(__global_state, inst, "RAY", raydir, " ", rframe, abcorr, obsrvr, 
	    &tol, (U_fp)gfstep_, (U_fp)gfrefn_, &__state->c_false, (U_fp)
	    gfrepi_, (U_fp)gfrepu_, (U_fp)gfrepf_, &__state->c_false, (L_fp)
	    gfbail_, cnfine, result, inst_len, (ftnlen)3, (ftnlen)1, 
	    rframe_len, abcorr_len, obsrvr_len);
    chkout_(__global_state, "GFRFOV", (ftnlen)6);
    return 0;
} /* gfrfov_ */

/* zzekllei.f -- translated by f2c (version 19991025).
...
extern zzekllei_init_t __zzekllei_init;
static inline zzekllei_state_t* get_zzekllei_state(cspice_t* state) {
	if (!state->zzekllei)
		state->zzekllei = __cspice_allocate_module(sizeof(
	zzekllei_state_t), &__zzekllei_init, sizeof(__zzekllei_init));
	return state->zzekllei;

}

/* $Procedure ZZEKLLEI ( EK, last less than or equal to, integer ) */
/* Subroutine */ int zzekllei_(cspice_t* __global_state, integer *handle, 
	integer *segdsc, integer *coldsc, integer *ikey, integer *prvloc, 
	integer *prvptr)
{
    ...
    /* Module state */
    zzekllei_state_t* __state = get_zzekllei_state(__global_state);
/* $ Abstract */
...
/*     If the column's not indexed, we have no business being here. */
	...
	chkin_(__global_state, "ZZEKLLEI", (ftnlen)8);
	...
	chkout_(__global_state, "ZZEKLLEI", (ftnlen)8);
	return 0;
    }
...
/*     Check the column's data type. */
	...
	zzekcnam_(__global_state, handle, coldsc, column, (ftnlen)32);
	chkin_(__global_state, "ZZEKLLEI", (ftnlen)8);
	setmsg_(__global_state, "Column # should be INT but has type #.", (
		ftnlen)38);
	errch_(__global_state, "#", column, (ftnlen)1, (ftnlen)32);
...
/*     greatest elements in the column. */
    ...
    if (zzekscmp_(__global_state, &__state->c__3, handle, segdsc, coldsc, &
	    begptr, &__state->c__1, &__state->c__3, " ", &__state->c_b17, 
	    ikey, &__state->c_false, (ftnlen)1)) {
...
/*        are less than or equal to the input value. */
	...
	*prvloc = 0;
	*prvptr = 0;
    } else if (zzekscmp_(__global_state, &__state->c__4, handle, segdsc, 
	    coldsc, &endptr, &__state->c__1, &__state->c__3, " ", &
	    __state->c_b17, ikey, &__state->c_false, (ftnlen)1)) {
...
/*           is halfway between BEGIN and END. */
	    ...
	    if (zzekscmp_(__global_state, &__state->c__4, handle, segdsc, 

/* ekrcec.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int ekrcec_(cspice_t* __global_state, integer *handle, 
	...
	cvals, logical *isnull, ftnlen column_len, ftnlen cvals_len)
{
    ...
    /* Module state */
    ekrcec_state_t* __state = get_ekrcec_state(__global_state);
/* $ Abstract */
...
/*     This column had better be of character type. */
    ...
    dtype = coldsc[1];
    if (dtype != 1) {
	chkin_(__global_state, "EKRCEC", (ftnlen)6);
	setmsg_(__global_state, "Column # is of type #; EKRCEC only works wi"
	...
	chkout_(__global_state, "EKRCEC", (ftnlen)6);
	return 0;
    }
...
/*        Look up the record pointer for the target record. */
	...
	zzekrd03_(__global_state, handle, segdsc, coldsc, &recptr, &cvlen, 
		cvals, isnull, cvals_len);
	*nvals = 1;
    } else if (class__ == 6) {
	zzektrdp_(__global_state, handle, &segdsc[6], recno, &recptr);
	*nvals = zzekesiz_(__global_state, handle, segdsc, coldsc, &recptr);
	zzekrd06_(__global_state, handle, segdsc, coldsc, &recptr, &
		__state->c__1, nvals, cvals, isnull, &found, cvals_len);
    } else if (class__ == 9) {

/*        Records in class 9 columns are identified by a record number */
/*        rather than a pointer. */

	zzekrd09_(__global_state, handle, segdsc, coldsc, recno, &cvlen, 
		cvals, isnull, cvals_len);
	*nvals = 1;
    } else {

...
static inline spkw08_state_t* get_spkw08_state(cspice_t* state) {
	if (!state->spkw08)
...
/* Subroutine */ int spkw08_(cspice_t* __global_state, integer *handle, 
	...
	frame_len, ftnlen segid_len)
{
    ...
    /* Module state */
    spkw08_state_t* __state = get_spkw08_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "SPKW08", (ftnlen)6);
...
/*     Get the NAIF integer code for the reference frame. */
	...
	chkout_(__global_state, "SPKW08", (ftnlen)6);
	return 0;
    }
...
/*     can be printed. */
	...
	if (chrcod < 32 || chrcod > 126) {
	    setmsg_(__global_state, "The segment identifier contains nonprin"
		    "table characters: ICHAR(SEGID(#:#))  = #", (ftnlen)79);
...
/*     in range. */
	...
	setmsg_(__global_state, "The interpolating polynomials have degree #"
		"; the valid degree range is [1, #].", (ftnlen)78);
...
/*     The segment stop time should be greater than the begin time. */
    ...
    if (*first >= *last) {
	setmsg_(__global_state, "The segment start time: # is greater than o"
		"r equal to the segment end time: #", (ftnlen)77);

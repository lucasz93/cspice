...
logical seti_(cspice_t* __global_state, integer *a, char *op, integer *b, 
	ftnlen op_len)
{
    ...
    /* Module state */
    seti_state_t* __state = get_seti_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SETI", (ftnlen)4);
	ret_val = FALSE_;
    }

/*     Obtain the cardinality of the sets. */

    carda = cardi_(__global_state, a);
    cardb = cardi_(__global_state, b);
...
/*     greater than or equal to the cardinality of B. */
	...
	if (carda >= cardb) {
	    ret_val = FALSE_;
	    chkout_(__global_state, "SETI", (ftnlen)4);
	    return ret_val;
	} else {
	    condlt = 0;
	    condeq = 1;

...
static inline zzspkgo1_state_t* get_zzspkgo1_state(cspice_t* state) {
	if (!state->zzspkgo1)
...
/* Subroutine */ int zzspkgo1_(cspice_t* __global_state, integer *targ, 
	...
	doublereal *lt, ftnlen ref_len)
{
    ...
    /* Module state */
    zzspkgo1_state_t* __state = get_zzspkgo1_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKGO1", (ftnlen)8);
    }
...
/*     same we can just fill in zero. */
	...
	*lt = 0.;
	...
	chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
	return 0;
    }
...
/*     should it change between the calls. */
	...
	if (frstnp_(__global_state, ref, ref_len) > 0) {
	    setmsg_(__global_state, "The string supplied to specify the refe"
		    ...
		    "an error in the call to ZZSPKGO1; 2. an uninitialized va"
		    "riable. ", (ftnlen)215);
...
/*           CTARG(I+1) and the state is called STEMP. */
	    ...
	    spkpvn_(__global_state, &handle, descr, et, &tframe[(i__1 = i__ - 
		    1) < 20 && 0 <= i__1 ? i__1 : s_rnge(&__global_state->f2c,
		     "tframe", i__1, "zzspkgo1_", (ftnlen)639)], &starg[(i__2 
		    = i__ * 6 - 6) < 120 && 0 <= i__2 ? i__2 : s_rnge(&
		    __global_state->f2c, "starg", i__2, "zzspkgo1_", (ftnlen)
		    639)], &ctarg[(i__3 = i__ - 1) < 20 && 0 <= i__3 ? i__3 : 
		    s_rnge(&__global_state->f2c, "ctarg", i__3, "zzspkgo1_", (
		    ftnlen)639)]);
...
/*              the last element of STARG. */
		...
		if (tframe[19] == tmpfrm) {
		    ...
		    zzfrmch1_(__global_state, &tframe[19], &tmpfrm, et, 
			    stxfrm);
		    if (failed_(__global_state)) {
			chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
			return 0;
...
/*     be zero if COBS is not found in CTARG. */
    ...
    if (ctarg[(i__1 = nct - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "ctarg", i__1, "zzspkgo1_", (ftnlen)775)] == 
	    cobs) {
	ctpos = nct;
	cframe = tframe[(i__1 = ctpos - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(&
		__global_state->f2c, "tframe", i__1, "zzspkgo1_", (ftnlen)777)
		];
    } else {
	ctpos = 0;
    }
...
/*              number of legs in the observer state is one or greater. */
		...
		zzfrmch1_(__global_state, &cframe, &tmpfrm, et, stxfrm);
		if (failed_(__global_state)) {
		    chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
		    return 0;
		}
...
/*     Compute the state of the target relative to CTARG(CTPOS) */
	...
	if (tframe[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(&
		__global_state->f2c, "tframe", i__2, "zzspkgo1_", (ftnlen)973)
		] == tframe[(i__3 = i__) < 20 && 0 <= i__3 ? i__3 : s_rnge(&
		__global_state->f2c, "tframe", i__3, "zzspkgo1_", (ftnlen)973)
		]) {
	    vaddg_(__global_state, &starg[(i__2 = i__ * 6 - 6) < 120 && 0 <= 
		    i__2 ? i__2 : s_rnge(&__global_state->f2c, "starg", i__2, 
		    "zzspkgo1_", (ftnlen)975)], &starg[(i__3 = (i__ + 1) * 6 
		    - 6) < 120 && 0 <= i__3 ? i__3 : s_rnge(&
		    __global_state->f2c, "starg", i__3, "zzspkgo1_", (ftnlen)
	    ...
	    zzfrmch1_(__global_state, &tframe[(i__2 = i__ - 1) < 20 && 0 <= 
		    i__2 ? i__2 : s_rnge(&__global_state->f2c, "tframe", i__2,
		     "zzspkgo1_", (ftnlen)988)], &tframe[(i__3 = i__) < 20 && 
		    0 <= i__3 ? i__3 : s_rnge(&__global_state->f2c, "tframe", 
		    i__3, "zzspkgo1_", (ftnlen)988)], et, stxfrm);
	    if (failed_(__global_state)) {
		chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
		return 0;
	    }
	    mxvg_(__global_state, stxfrm, &starg[(i__2 = i__ * 6 - 6) < 120 &&
		    ...
		    i__2, "zzspkgo1_", (ftnlen)995)], &__state->c__6, &
		    __state->c__6, stemp);
	    vaddg_(__global_state, stemp, &starg[(i__2 = (i__ + 1) * 6 - 6) < 
		    120 && 0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, 
		    "starg", i__2, "zzspkgo1_", (ftnlen)996)], &__state->c__6,
		     vtemp);
	    moved_(__global_state, vtemp, &__state->c__6, &starg[(i__2 = (i__ 
		    + 1) * 6 - 6) < 120 && 0 <= i__2 ? i__2 : s_rnge(&
		    __global_state->f2c, "starg", i__2, "zzspkgo1_", (ftnlen)
...
/*     frame transformations. */
    ...
    if (tframe[(i__1 = ctpos - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "tframe", i__1, "zzspkgo1_", (ftnlen)1010)] 
	    == cframe) {
	vsubg_(__global_state, &starg[(i__1 = ctpos * 6 - 6) < 120 && 0 <= 
		i__1 ? i__1 : s_rnge(&__global_state->f2c, "starg", i__1, 
		"zzspkgo1_", (ftnlen)1012)], sobs, &__state->c__6, state);
    } else if (tframe[(i__1 = ctpos - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "tframe", i__1, "zzspkgo1_", (ftnlen)1014)] 
	    == refid) {
...
/*        of the observer from the state of the target. */
	...
	if (refid > 0 && refid <= 21 && cframe > 0 && cframe <= 21) {
	    ...
	    zzfrmch1_(__global_state, &cframe, &refid, et, stxfrm);
	    if (failed_(__global_state)) {
		chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
		return 0;
	    }
...
/*        Set CFRAME to reflect this. */
	...
	vsubg_(__global_state, &starg[(i__1 = ctpos * 6 - 6) < 120 && 0 <= 
		i__1 ? i__1 : s_rnge(&__global_state->f2c, "starg", i__1, 
		"zzspkgo1_", (ftnlen)1046)], stemp, &__state->c__6, state);
    } else if (cframe > 0 && cframe <= 21 && tframe[(i__1 = ctpos - 1) < 20 &&
	     0 <= i__1 ? i__1 : s_rnge(&__global_state->f2c, "tframe", i__1, 
	    "zzspkgo1_", (ftnlen)1049)] > 0 && tframe[(i__1 = ctpos - 1) < 20 
	    && 0 <= i__1 ? i__1 : s_rnge(&__global_state->f2c, "tframe", i__1,
	     "zzspkgo1_", (ftnlen)1049)] <= 21) {
...
/*        transformation. */
	...
	zzfrmch1_(__global_state, &tframe[(i__1 = ctpos - 1) < 20 && 0 <= 
		i__1 ? i__1 : s_rnge(&__global_state->f2c, "tframe", i__1, 
		"zzspkgo1_", (ftnlen)1065)], &cframe, et, stxfrm);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKGO1", (ftnlen)8);
	    return 0;
	}
	mxvg_(__global_state, stxfrm, &starg[(i__1 = ctpos * 6 - 6) < 120 && 
		0 <= i__1 ? i__1 : s_rnge(&__global_state->f2c, "starg", i__1,
		 "zzspkgo1_", (ftnlen)1072)], &__state->c__6, &__state->c__6, 
		stemp);
	vsubg_(__global_state, stemp, sobs, &__state->c__6, state);
    }

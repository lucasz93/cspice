...
/* Subroutine */ int zzdynvai_(cspice_t* __global_state, char *frname, 
	integer *frcode, char *item, integer *maxn, integer *n, integer *
	values, ftnlen frname_len, ftnlen item_len)
{
    ...
    /* Module state */
    zzdynvai_state_t* __state = get_zzdynvai_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling */
    ...
    chkin_(__global_state, "ZZDYNVAI", (ftnlen)8);
...
/*     Convert the frame code to a string. */
	...
	chkout_(__global_state, "ZZDYNVAI", (ftnlen)8);
	return 0;
    }
...
/*     error message if need be. */
	...
	setmsg_(__global_state, "The kernel variable # has used to define fr"
		"ame # was expected to have integer data type but in fact has"
		" character type.  Usually this type of problem is due to an "
		"error in a frame definition provided in a frame kernel.", (
		ftnlen)218);
	errch_(__global_state, "#", kvname, (ftnlen)1, (ftnlen)32);
	errch_(__global_state, "#", frname, (ftnlen)1, frname_len);
...
/*     Look up the kernel variable. */
    ...
    gipool_(__global_state, kvname, &__state->c__1, maxn, n, values, &found, (
	    ftnlen)32);
    if (! found) {

...
static inline zzfrmch0_state_t* get_zzfrmch0_state(cspice_t* state) {
	if (!state->zzfrmch0)
...
/* Subroutine */ int zzfrmch0_(cspice_t* __global_state, integer *frame1, 
	integer *frame2, doublereal *et, doublereal *xform)
{
    ...
    logical done;
    integer cent;
    extern /* Subroutine */ int zzfrmgt0_(cspice_t*, integer *, doublereal *, 
    ...
    /* Module state */
    zzfrmch0_state_t* __state = get_zzfrmch0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "ZZFRMCH0", (ftnlen)8);
...
/*     same then we simply return the identity matrix. */
	...
	for (i__ = 1; i__ <= 6; ++i__) {
	    xform[(i__1 = i__ + i__ * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
		    s_rnge(&__global_state->f2c, "xform", i__1, "zzfrmch0_", (
		    ftnlen)371)] = 1.;
	    i__1 = i__ - 1;
	    for (j = 1; j <= i__1; ++j) {
	...
	chkout_(__global_state, "ZZFRMCH0", (ftnlen)8);
	return 0;
    }
...
/*     frames are recognized. */
    ...
    frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "zzfrmch0_", (ftnlen)418)] = *
	    frame1;
    found = TRUE_;
...
/*     one that transforms to J2000 (frame id = 1) or we hit FRAME2. */
    ...
    while(frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "zzfrmch0_", (ftnlen)424)] != 
	    1 && node < 10 && frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? 
	    i__2 : s_rnge(&__global_state->f2c, "frame", i__2, "zzfrmch0_", (
	    ftnlen)424)] != *frame2 && found) {

/*        Find out what transformation is available for this */
/*        frame. */

	zzfrmgt0_(__global_state, &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ?
		 i__1 : s_rnge(&__global_state->f2c, "frame", i__1, "zzfrmch"
		"0_", (ftnlen)432)], et, &trans[(i__2 = (node * 6 + 1) * 6 - 
		...
		"frame", i__3, "zzfrmch0_", (ftnlen)432)], &found);
	if (found) {
...
/*           for the next NODE. */
	    ...
	    __global_state->f2c, "frame", i__1, "zzfrmch0_", (ftnlen)448)] == 
	    1 || frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? i__2 : s_rnge(&
	    __global_state->f2c, "frame", i__2, "zzfrmch0_", (ftnlen)448)] == 
	    *frame2 || ! found;
    while(! done) {
...
/*        root classes or we run into FRAME2. */
	...
	zzfrmgt0_(__global_state, &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ?
		 i__1 : s_rnge(&__global_state->f2c, "frame", i__1, "zzfrmch"
		"0_", (ftnlen)462)], et, &trans[(i__2 = (node * 6 + 1) * 6 - 
		42) < 504 && 0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, 
...
/*        another transformation. */
		...
		__global_state->f2c, "frame", i__1, "zzfrmch0_", (ftnlen)488)]
		 == 1 || frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? i__2 : 
		s_rnge(&__global_state->f2c, "frame", i__2, "zzfrmch0_", (
...
/*     We now have to do essentially the same thing for FRAME2. */
    ...
    if (frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "zzfrmch0_", (ftnlen)526)] == 
	    *frame2) {
	...
	for (k = 1; k <= 2; ++k) {
		...
		for (j = 4; j <= 6; ++j) {
		    trans2[(i__1 = i__ + (j + k * 6) * 6 - 43) < 72 && 0 <= 
			    ...
			    "s2", i__1, "zzfrmch0_", (ftnlen)563)] = 0.;
		}
	    }
	}
	put = 1;
...
/*           directly into TRANS2(,,PUT). */
	    ...
	    zzfrmgt0_(__global_state, &this__, et, &trans2[(i__1 = (put * 6 + 
		    1) * 6 - 42) < 72 && 0 <= i__1 ? i__1 : s_rnge(&
		    __global_state->f2c, "trans2", i__1, "zzfrmch0_", (ftnlen)
		    592)], &relto, &found);
	    if (found) {
		this__ = relto;
		get = put;
...
/*           Fetch the transformation into a temporary spot TEMPXF */
	    ...
	    zzfrmgt0_(__global_state, &this__, et, tempxf, &relto, &found);
	    if (found) {
...
/*              works see ZZMSXF. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			trans2[(i__1 = i__ + (j + put * 6) * 6 - 43) < 72 && 
				...
				"h0_", (ftnlen)621)] = tempxf[(i__2 = i__ - 1) 
				< 36 && 0 <= i__2 ? i__2 : s_rnge(&
				__global_state->f2c, "tempxf", i__2, "zzfrmc"
				"h0_", (ftnlen)621)] * trans2[(i__3 = (j + get 
				* 6) * 6 - 42) < 72 && 0 <= i__3 ? i__3 : 
				...
				"h0_", (ftnlen)621)] * trans2[(i__5 = (j + get 
				* 6) * 6 - 41) < 72 && 0 <= i__5 ? i__5 : 
				...
				"h0_", (ftnlen)621)] * trans2[(i__7 = (j + get 
		...
		for (i__ = 4; i__ <= 6; ++i__) {
			...
			trans2[(i__1 = i__ + (j + put * 6) * 6 - 43) < 72 && 
				...
				* 6) * 6 - 39) < 72 && 0 <= i__9 ? i__9 : 
				...
				"mch0_", (ftnlen)630)] * trans2[(i__11 = (j + 
...
/*     get from FRAME1 to FRAME2. */
	...
	zznofcon_(__global_state, et, frame1, &frame[(i__1 = node - 1) < 10 &&
		...
		i__1, "zzfrmch0_", (ftnlen)692)], frame2, &this__, errmsg, (
		ftnlen)1840);
...
/*     result from FRAME1 to FRAME2. */
    ...
    invstm_(__global_state, &trans2[(i__1 = (get * 6 + 1) * 6 - 42) < 72 && 0 
	    ...
	    "zzfrmch0_", (ftnlen)735)], &trans[(i__2 = (cmnode * 6 + 1) * 6 - 
	    42) < 504 && 0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, 

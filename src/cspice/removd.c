...
static inline removd_state_t* get_removd_state(cspice_t* state) {
	return 0;
}

/* $Procedure      REMOVD ( Remove an item from a double precision set ) */
/* Subroutine */ int removd_(cspice_t* __global_state, doublereal *item, 
	doublereal *a)
{
    ...
    /* Module state */
    removd_state_t* __state = get_removd_state(__global_state);
/* $ Abstract */
...
/*     Standard error handling: */
	...
	chkin_(__global_state, "REMOVD", (ftnlen)6);
    }
...
/*     Determine the location (if any) of the item within the set. */
    ...
    loc = bsrchd_(__global_state, item, &card, &a[6]);
...
/*        future reference. */
    ...
    chkout_(__global_state, "REMOVD", (ftnlen)6);
    return 0;
} /* removd_ */

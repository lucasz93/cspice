...
static inline zzspkzp0_state_t* get_zzspkzp0_state(cspice_t* state) {
	if (!state->zzspkzp0)
...
/* Subroutine */ int zzspkzp0_(cspice_t* __global_state, integer *targ, 
	...
	ptarg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int zzspkpa0_(cspice_t*, integer *, doublereal *, 
    ...
    /* Module state */
    zzspkzp0_state_t* __state = get_zzspkzp0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKZP0", (ftnlen)8);
    }
...
/*     with the requested aberration corrections. */
	...
	zzspkgp0_(__global_state, targ, et, ref, obs, ptarg, lt, ref_len);
    } else {
...
/*        frame. */
	...
	if (__state->reqfrm == 0) {
	    ...
	    chkout_(__global_state, "ZZSPKZP0", (ftnlen)8);
	    return 0;
	}
...
/*        call SPKSSB, SPKAPO and return. */
	...
	if (__state->type__ == 1) {
	    zzspksb0_(__global_state, obs, et, ref, __state->sobs, ref_len);
	    zzspkpa0_(__global_state, targ, et, ref, __state->sobs, abcorr, 
...
/*        We also need the light time to the center of the frame. */
	...
	zzspksb0_(__global_state, obs, et, "J2000", __state->sobs, (ftnlen)5);
	zzspkpa0_(__global_state, targ, et, "J2000", __state->sobs, abcorr, 
	...
	if (__state->center == *obs) {
	    ...
	    zzspkpa0_(__global_state, &__state->center, et, "J2000", 
...
/*        and convert the position. */
	...
	zzrefch0_(__global_state, &__state->fj2000, &__state->reqfrm, &d__1, 
		__state->xform);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKZP0", (ftnlen)8);
	    return 0;

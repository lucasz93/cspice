...
/* Subroutine */ int spks18_(cspice_t* __global_state, integer *handle, 
	integer *baddr, integer *eaddr, doublereal *begin, doublereal *end)
{
    ...
    /* Module state */
    spks18_state_t* __state = get_spks18_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SPKS18", (ftnlen)6);
    }

/*     See whether there's any work to do; return immediately if not. */

    if (*begin > *end) {
	chkout_(__global_state, "SPKS18", (ftnlen)6);
	return 0;
    }
...
/*     size." Also get the number of records in the segment. */
    ...
    subtyp = i_dnnt(&__global_state->f2c, data);
    winsiz = i_dnnt(&__global_state->f2c, &data[1]);
    nrec = i_dnnt(&__global_state->f2c, &data[2]);
...
/*     Copy states REC(1) through REC(2) to the output file. */
	...
	dafada_(__global_state, data, &packsz);
    }

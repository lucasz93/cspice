/* remlad.f -- translated by f2c (version 19991025).
...
typedef int remlad_state_t;
static inline remlad_state_t* get_remlad_state(cspice_t* state) {
	return 0;
}

/* $Procedure      REMLAD (Remove elements from a double precision array) */
/* Subroutine */ int remlad_(cspice_t* __global_state, integer *ne, integer *
	loc, doublereal *array, integer *na)
{
    ...
    /* Module state */
    remlad_state_t* __state = get_remlad_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "REMLAD", (ftnlen)6);
    }
...
/*     value of LOC is invalid, and an error is signalled. */
	...
	chkout_(__global_state, "REMLAD", (ftnlen)6);
	return 0;

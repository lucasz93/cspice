...
static inline zzeksz06_state_t* get_zzeksz06_state(cspice_t* state) {
	return 0;
}

/* $Procedure ZZEKSZ06 ( EK, element entry size, class 6 ) */
integer zzeksz06_(cspice_t* __global_state, integer *handle, integer *segdsc, 
	integer *coldsc, integer *recptr)
{
    ...
    /* Module state */
    zzeksz06_state_t* __state = get_zzeksz06_state(__global_state);
/* $ Abstract */
...
/*     Make sure the column exists. */
	...
	chkin_(__global_state, "ZZEKSZ06", (ftnlen)8);
	...
	chkout_(__global_state, "ZZEKSZ06", (ftnlen)8);
	return ret_val;
    }
...
/*        DATPTR points to the element count. */
	...
	zzekgei_(__global_state, handle, &datptr, &ret_val);
    }
    return ret_val;
} /* zzeksz06_ */

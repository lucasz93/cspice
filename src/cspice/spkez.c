/* spkez.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int spkez_(cspice_t* __global_state, integer *targ, 
	...
	starg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkez_state_t* __state = get_spkez_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "SPKEZ", (ftnlen)5);
...
/*        had on the previous call, if any. Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "SPKEZ", (ftnlen)5);
	    return 0;
	}
...
/*     to the frame designated by REF. */
    ...
    if (__state->usegeo) {
	spkgeo_(__global_state, targ, et, ref, obs, starg, lt, ref_len);
    } else {
...
/*        call SPKACS and return. */
	...
	if (__state->type__ == 1) {
	    spkacs_(__global_state, targ, et, ref, abcorr, obs, starg, lt, &
...
/*        of the target relative to the observer. */
	...
	spkacs_(__global_state, targ, et, "J2000", abcorr, obs, 
...
/*        So we'll scale the derivative block of XFORM accordingly. */
	...
	for (__state->i__ = 1; __state->i__ <= 3; ++__state->i__) {
	    ...
	    vsclip_(__global_state, &d__1, &__state->xform[(i__1 = 
		    ...
		    __global_state->f2c, "xform", i__1, "spkez_", (ftnlen)
		    1329)]);
	}

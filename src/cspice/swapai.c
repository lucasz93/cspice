...
static inline swapai_state_t* get_swapai_state(cspice_t* state) {
	return 0;
}

/* $Procedure      SWAPAI ( Swap elements within an integer array ) */
/* Subroutine */ int swapai_(cspice_t* __global_state, integer *n, integer *
	locn, integer *m, integer *locm, integer *array)
{
    ...
    integer begsub;
    integer direct;
    extern /* Subroutine */ int cyaiip_(cspice_t*, integer *, char *, integer 
	    *, integer *, ftnlen);
    ...
    /* Module state */
    swapai_state_t* __state = get_swapai_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SWAPAI", (ftnlen)6);
    }
...
/*     Check to see if the inputs are valid. */
	...
	chkout_(__global_state, "SWAPAI", (ftnlen)6);
	return 0;
    } else if (*m < 0) {
...
/*     Direct exchange. */
	...
	swapi_(__global_state, &array[ln + i__ - 1], &array[lm + i__ - 1]);
    }
...
/*     Cycle. */
	...
	cyaiip_(__global_state, &nsub, dir, &extra, &array[begsub - 1], (
		ftnlen)1);
    }
    chkout_(__global_state, "SWAPAI", (ftnlen)6);
    return 0;

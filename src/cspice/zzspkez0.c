...
extern zzspkez0_init_t __zzspkez0_init;
static inline zzspkez0_state_t* get_zzspkez0_state(cspice_t* state) {
	if (!state->zzspkez0)
		state->zzspkez0 = __cspice_allocate_module(sizeof(
	zzspkez0_state_t), &__zzspkez0_init, sizeof(__zzspkez0_init));
	return state->zzspkez0;

}

/* $Procedure ZZSPKEZ0 ( S/P Kernel, easy reader ) */
/* Subroutine */ int zzspkez0_(cspice_t* __global_state, integer *targ, 
	...
	starg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int zzspkac0_(cspice_t*, integer *, doublereal *, 
	    char *, char *, integer *, doublereal *, doublereal *, doublereal 
	    *, ftnlen, ftnlen);
    extern /* Subroutine */ int zzspkgo0_(cspice_t*, integer *, doublereal *, 
    ...
    extern /* Subroutine */ int zzspklt0_(cspice_t*, integer *, doublereal *, 
    ...
    /* Module state */
    zzspkez0_state_t* __state = get_zzspkez0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "ZZSPKEZ0", (ftnlen)8);
...
/*        had on the previous call, if any. Analyze the new flag. */
	...
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKEZ0", (ftnlen)8);
	    return 0;
	}
...
/*     to the frame designated by REF. */
    ...
    if (__state->usegeo) {
	zzspkgo0_(__global_state, targ, et, ref, obs, starg, lt, ref_len);
    } else {
...
/*        call SPKACS and return. */
	...
	if (__state->type__ == 1) {
	    zzspkac0_(__global_state, targ, et, ref, abcorr, obs, starg, lt, &
...
/*        of the target relative to the observer. */
	...
	zzspkac0_(__global_state, targ, et, "J2000", abcorr, obs, 
	...
	if (__state->center == *obs) {
	    ...
	    zzspksb0_(__global_state, obs, et, "J2000", __state->stobs, (
		    ftnlen)5);
	    zzspklt0_(__global_state, &__state->center, et, "J2000", abcorr, 
...
/*        and convert the state. */
	...
	zzfrmch0_(__global_state, &__state->fj2000, &__state->reqfrm, &d__1, 
		__state->xform);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKEZ0", (ftnlen)8);
	    return 0;
...
/*        So we'll scale the derivative block of XFORM accordingly. */
	...
	for (__state->i__ = 1; __state->i__ <= 3; ++__state->i__) {
	    ...
	    vsclip_(__global_state, &d__1, &__state->xform[(i__1 = 
		    ...
		    __global_state->f2c, "xform", i__1, "zzspkez0_", (ftnlen)
		    1330)]);
	}

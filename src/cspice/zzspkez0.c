...
static zzspkez0_state_t* get_zzspkez0_state() {
	...
	ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspkez0_state_t* __state = get_zzspkez0_state();
/* $ Abstract */
...
/*        call SPKACS and return. */
	...
	if (__state->type__ == 1) {
	    zzspkac0_(targ, et, ref, abcorr, obs, starg, lt, &__state->dlt, 
		    ref_len, abcorr_len);
	    chkout_("ZZSPKEZ0", (ftnlen)8);
	    return 0;
	}
...
/*        of the target relative to the observer. */
	...
	zzspkac0_(targ, et, "J2000", abcorr, obs, __state->state, lt, &
	...
	if (__state->center == *obs) {
	    ...
	    zzspksb0_(obs, et, "J2000", __state->stobs, (ftnlen)5);
	    zzspklt0_(&__state->center, et, "J2000", abcorr, __state->stobs, 
...
/*        and convert the state. */
	...
	zzfrmch0_(&__state->fj2000, &__state->reqfrm, &d__1, __state->xform);
	if (failed_()) {
	    chkout_("ZZSPKEZ0", (ftnlen)8);
	    return 0;
	}
...
/*        So we'll scale the derivative block of XFORM accordingly. */
	...
	for (__state->i__ = 1; __state->i__ <= 3; ++__state->i__) {
	    ...
	    vsclip_(&d__1, &__state->xform[(i__1 = __state->i__ * 6 - 3) < 36 
		    && 0 <= i__1 ? i__1 : s_rnge("xform", i__1, "zzspkez0_", (
		    ftnlen)1330)]);
	}

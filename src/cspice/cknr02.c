/* cknr02.f -- translated by f2c (version 19991025).
...
extern cknr02_init_t __cknr02_init;
static inline cknr02_state_t* get_cknr02_state(cspice_t* state) {
	if (!state->cknr02)
		state->cknr02 = __cspice_allocate_module(sizeof(
	cknr02_state_t), &__cknr02_init, sizeof(__cknr02_init));
	return state->cknr02;

}

/* $Procedure      CKNR02 ( C-kernel, number of records, type 02 ) */
/* Subroutine */ int cknr02_(cspice_t* __global_state, integer *handle, 
	doublereal *descr, integer *nrec)
{
    ...
    /* Module state */
    cknr02_state_t* __state = get_cknr02_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "CKNR02", (ftnlen)6);
    }
...
/*     If this segment is not of data type 2, then signal an error. */
	...
	chkout_(__global_state, "CKNR02", (ftnlen)6);
	return 0;
    }

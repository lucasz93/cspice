...
static inline syselc_state_t* get_syselc_state(cspice_t* state) {
	return 0;
}

/* $Procedure      SYSELC ( Select a subset of the values of a symbol ) */
/* Subroutine */ int syselc_(cspice_t* __global_state, char *name__, integer *
	begin, integer *end, char *tabsym, integer *tabptr, char *tabval, 
	char *values, logical *found, ftnlen name_len, ftnlen tabsym_len, 
	ftnlen tabval_len, ftnlen values_len)
{
    ...
    /* Module state */
    syselc_state_t* __state = get_syselc_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SYSELC", (ftnlen)6);
    }
...
/*        Does this request even make sense? */
	...
	if (*begin >= 1 && *begin <= n && *end >= 1 && *end <= n && *begin <= 
	    ...
	    *found = TRUE_;
	    ...
	    i__1 = *end - *begin + 1;
	    movec_(__global_state, tabval + (locval + *begin + 4) * 
		    tabval_len, &i__1, values, tabval_len, values_len);
	} else {
	    *found = FALSE_;
	}
    }
    chkout_(__global_state, "SYSELC", (ftnlen)6);
    return 0;
} /* syselc_ */

...
/* Subroutine */ int sydupi_(cspice_t* __global_state, char *name__, char *
	copy, char *tabsym, integer *tabptr, integer *tabval, ftnlen name_len,
	 ftnlen copy_len, ftnlen tabsym_len)
{
    ...
    /* Module state */
    sydupi_state_t* __state = get_sydupi_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling */
	...
	chkin_(__global_state, "SYDUPI", (ftnlen)6);
    }
...
/*     If the original symbol is not in the table, we can't make a copy. */
    ...
    if (! oldsym[0]) {
	setmsg_(__global_state, "SYDUPI: The symbol to be duplicated, #, is "
...
/*        Can we make a copy without overflow? */
	...
	if (nsym + newsym > sizec_(__global_state, tabsym, tabsym_len)) {
	    setmsg_(__global_state, "SYDUPI: Duplication of the symbol # cau"
	...
	} else if (nval + newval > sizei_(__global_state, tabval)) {
	    setmsg_(__global_state, "SYDUPI: Duplication of the symbol # cau"
...
/*           dimension to the name and pointer tables. */
	    ...
	    if (dimval[1] > 0) {
		remlai_(__global_state, &dimval[1], &locval[1], &tabval[6], &
			nval);
...
/*           copy is earlier in the table than the original.) */
	    ...
	    i__1 = nval + dimval[0];
	    scardi_(__global_state, &i__1, tabval);
	}
    }
    chkout_(__global_state, "SYDUPI", (ftnlen)6);
    return 0;
} /* sydupi_ */

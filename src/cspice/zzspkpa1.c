...
static inline zzspkpa1_state_t* get_zzspkpa1_state(cspice_t* state) {
	if (!state->zzspkpa1)
...
/* Subroutine */ int zzspkpa1_(cspice_t* __global_state, integer *targ, 
	...
	*ptarg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspkpa1_state_t* __state = get_zzspkpa1_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKPA1", (ftnlen)8);
    }
...
/*        Locate the flag in our list of flags. */
	...
	if (i__ == 0) {
	    ...
	    chkout_(__global_state, "ZZSPKPA1", (ftnlen)8);
	    return 0;
	}
...
/*     light time. */
    ...
    zzspkgp1_(__global_state, targ, et, ref, &__state->c__0, ptarg, lt, 
	    ref_len);
...
/*     the observer remains where he is. */
	...
	d__1 = *et + ltsign * *lt;
	zzspkgp1_(__global_state, targ, &d__1, ref, &__state->c__0, ptarg, lt,
		 ref_len);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "ZZSPKPA1", (ftnlen)8);
	    return 0;

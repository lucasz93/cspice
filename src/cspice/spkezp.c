/* spkezp.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int spkezp_(cspice_t* __global_state, integer *targ, 
	...
	ptarg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int spkapo_(cspice_t*, integer *, doublereal *, 
    ...
    /* Module state */
    spkezp_state_t* __state = get_spkezp_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SPKEZP", (ftnlen)6);
    }
...
/*     with the requested aberration corrections. */
	...
	spkgps_(__global_state, targ, et, ref, obs, ptarg, lt, ref_len);
    } else {
...
/*        frame. */
	...
	if (__state->reqfrm == 0) {
	    ...
	    chkout_(__global_state, "SPKEZP", (ftnlen)6);
	    return 0;
	}
...
/*        call SPKSSB, SPKAPO and return. */
	...
	if (__state->type__ == 1) {
	    spkssb_(__global_state, obs, et, ref, __state->sobs, ref_len);
	    spkapo_(__global_state, targ, et, ref, __state->sobs, abcorr, 
...
/*        We also need the light time to the center of the frame. */
	...
	spkssb_(__global_state, obs, et, "J2000", __state->sobs, (ftnlen)5);
	spkapo_(__global_state, targ, et, "J2000", __state->sobs, abcorr, 
	...
	if (__state->center == *obs) {
	    ...
	    spkapo_(__global_state, &__state->center, et, "J2000", 
...
/*        and convert the position. */
	...
	refchg_(__global_state, &__state->fj2000, &__state->reqfrm, &d__1, 
		__state->xform);
	if (failed_(__global_state)) {
	    chkout_(__global_state, "SPKEZP", (ftnlen)6);
	    return 0;

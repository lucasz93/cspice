...
static spkezp_state_t* get_spkezp_state() {
	...
	ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkezp_state_t* __state = get_spkezp_state();
/* $ Abstract */
...
/*        call SPKSSB, SPKAPO and return. */
	...
	if (__state->type__ == 1) {
	    spkssb_(obs, et, ref, __state->sobs, ref_len);
	    spkapo_(targ, et, ref, __state->sobs, abcorr, ptarg, lt, ref_len, 
		    abcorr_len);
	    chkout_("SPKEZP", (ftnlen)6);
	    return 0;
	}
...
/*        We also need the light time to the center of the frame. */
	...
	spkssb_(obs, et, "J2000", __state->sobs, (ftnlen)5);
	spkapo_(targ, et, "J2000", __state->sobs, abcorr, __state->postn, lt, 
		(ftnlen)5, abcorr_len);
	if (failed_()) {
	    chkout_("SPKEZP", (ftnlen)6);
	    return 0;
	...
	if (__state->center == *obs) {
	    ...
	    spkapo_(&__state->center, et, "J2000", __state->sobs, abcorr, 
...
/*        and convert the position. */
	...
	refchg_(&__state->fj2000, &__state->reqfrm, &d__1, __state->xform);
	if (failed_()) {
	    chkout_("SPKEZP", (ftnlen)6);
	    return 0;
	}

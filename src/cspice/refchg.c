...
/* Subroutine */ int refchg_(cspice_t* __global_state, integer *frame1, 
	integer *frame2, doublereal *et, doublereal *rotate)
{
    ...
    extern /* Subroutine */ int rotget_(cspice_t*, integer *, doublereal *, 
    ...
    /* Module state */
    refchg_state_t* __state = get_refchg_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "REFCHG", (ftnlen)6);
...
/*     same then we simply return the identity matrix. */
	...
	chkout_(__global_state, "REFCHG", (ftnlen)6);
	return 0;
    }
...
/*     frames are recognized. */
    ...
    frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "refchg_", (ftnlen)287)] = *
	    frame1;
    found = TRUE_;
...
/*     one that rotates to J2000 (frame id = 1) or we hit FRAME2. */
    ...
    while(frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "refchg_", (ftnlen)293)] != 1 
	    && node < 10 && frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? i__2 :
	     s_rnge(&__global_state->f2c, "frame", i__2, "refchg_", (ftnlen)
	    293)] != *frame2 && found) {

/*        Find out what rotation is available for this */
/*        frame. */

	rotget_(__global_state, &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? 
		i__1 : s_rnge(&__global_state->f2c, "frame", i__1, "refchg_", 
		(ftnlen)301)], et, &rot[(i__2 = (node * 3 + 1) * 3 - 12) < 
		126 && 0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, "rot", 
		i__2, "refchg_", (ftnlen)301)], &frame[(i__3 = node) < 10 && 
		0 <= i__3 ? i__3 : s_rnge(&__global_state->f2c, "frame", i__3,
		 "refchg_", (ftnlen)301)], &found);
	if (found) {
...
/*           for the next NODE. */
	    ...
	    __global_state->f2c, "frame", i__1, "refchg_", (ftnlen)317)] == 1 
	    || frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? i__2 : s_rnge(&
	    __global_state->f2c, "frame", i__2, "refchg_", (ftnlen)317)] == *
	    frame2 || ! found;
    while(! done) {
...
/*        root classes or we run into FRAME2. */
	...
	rotget_(__global_state, &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? 
		i__1 : s_rnge(&__global_state->f2c, "frame", i__1, "refchg_", 
		(ftnlen)331)], et, &rot[(i__2 = (node * 3 + 1) * 3 - 12) < 
		126 && 0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, "rot", 
		i__2, "refchg_", (ftnlen)331)], &relto, &found);
	if (found) {
...
/*        another rotation. */
		...
		__global_state->f2c, "frame", i__1, "refchg_", (ftnlen)357)] 
		== 1 || frame[(i__2 = node - 1) < 10 && 0 <= i__2 ? i__2 : 
		s_rnge(&__global_state->f2c, "frame", i__2, "refchg_", (
...
/*     We now have to do essentially the same thing for FRAME2. */
    ...
    if (frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(&
	    __global_state->f2c, "frame", i__1, "refchg_", (ftnlen)395)] == *
	    frame2) {
...
/*           directly into ROT2(,,PUT). */
	    ...
	    rotget_(__global_state, &this__, et, &rot2[(i__1 = (put * 3 + 1) *
		     3 - 12) < 18 && 0 <= i__1 ? i__1 : s_rnge(&
		    __global_state->f2c, "rot2", i__1, "refchg_", (ftnlen)452)
		    ], &relto, &found);
	    if (found) {
		this__ = relto;
		get = put;
...
/*           Fetch the rotation into a temporary spot TMPROT */
	    ...
	    rotget_(__global_state, &this__, et, tmprot, &relto, &found);
	    if (found) {
...
/*              product (in ROT2(,,GET) ).  We do this in line. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			rot2[(i__1 = i__ + (j + put * 3) * 3 - 13) < 18 && 0 
				...
				i__3, "refchg_", (ftnlen)478)] + tmprot[(i__4 
				= i__ + 2) < 9 && 0 <= i__4 ? i__4 : s_rnge(&
				__global_state->f2c, "tmprot", i__4, "refchg_"
				, (ftnlen)478)] * rot2[(i__5 = (j + get * 3) *
				 3 - 11) < 18 && 0 <= i__5 ? i__5 : s_rnge(&
				__global_state->f2c, "rot2", i__5, "refchg_", 
				(ftnlen)478)] + tmprot[(i__6 = i__ + 5) < 9 &&
				 0 <= i__6 ? i__6 : s_rnge(&
				__global_state->f2c, "tmprot", i__6, "refchg_"
				, (ftnlen)478)] * rot2[(i__7 = (j + get * 3) *
				 3 - 10) < 18 && 0 <= i__7 ? i__7 : s_rnge(&
				__global_state->f2c, "rot2", i__7, "refchg_", 
				(ftnlen)478)];
		    }
		}
...
/*     get from FRAME1 to FRAME2. */
	...
	zznofcon_(__global_state, et, frame1, &frame[(i__1 = node - 1) < 10 &&
		...
		i__1, "refchg_", (ftnlen)525)], frame2, &this__, errmsg, (
		ftnlen)1840);
...
/*     result from FRAME1 to FRAME2. */
    ...
    xpose_(__global_state, &rot2[(i__1 = (get * 3 + 1) * 3 - 12) < 18 && 0 <= 
	    i__1 ? i__1 : s_rnge(&__global_state->f2c, "rot2", i__1, "refchg_"
	    , (ftnlen)568)], &rot[(i__2 = (cmnode * 3 + 1) * 3 - 12) < 126 && 
	    0 <= i__2 ? i__2 : s_rnge(&__global_state->f2c, "rot", i__2, 
	    "refchg_", (ftnlen)568)]);

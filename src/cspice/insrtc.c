...
static inline insrtc_state_t* get_insrtc_state(cspice_t* state) {
	return 0;
}

/* $Procedure      INSRTC ( Insert an item into a character set ) */
/* Subroutine */ int insrtc_(cspice_t* __global_state, char *item, char *a, 
	ftnlen item_len, ftnlen a_len)
{
    ...
    /* Module state */
    insrtc_state_t* __state = get_insrtc_state(__global_state);
/* $ Abstract */
...
/*     Set up the error processing. */
    ...
    chkin_(__global_state, "INSRTC", (ftnlen)6);

/*     What are the size and cardinality of the set? */

    size = sizec_(__global_state, a, a_len);
...
/* Computing MIN */
    i__1 = i_len(&__global_state->f2c, item, item_len), i__2 = i_len(&
	    __global_state->f2c, a + a_len * 6, a_len);
    slen = min(i__1,i__2);
...
/*     element of the set. */
    ...
    last = lstlec_(__global_state, item, &card, a + a_len * 6, slen, a_len);

/*     Is the item already in the set? If not, it needs to be inserted. */

    if (last > 0) {
	in = s_cmp(&__global_state->f2c, a + (last + 5) * a_len, item, a_len, 
		slen) == 0;
    } else {
	in = FALSE_;
    }
...
/*        cardinality for future reference. */
	...
	if (card < size) {
		...
		s_copy(&__global_state->f2c, a + (i__ + 6) * a_len, a + (i__ 
			+ 5) * a_len, a_len, a_len);
	    }
	    s_copy(&__global_state->f2c, a + (last + 6) * a_len, item, a_len, 
		    slen);
	    i__1 = card + 1;
    ...
    chkout_(__global_state, "INSRTC", (ftnlen)6);
    return 0;
} /* insrtc_ */

...
static inline dafb2t_state_t* get_dafb2t_state(cspice_t* state) {
	if (!state->dafb2t)
...
/* Subroutine */ int dafb2t_(cspice_t* __global_state, char *binary, integer *
	text, ftnlen binary_len)
{
    ...
    /* Subroutine */ int s_cat(f2c_state_t*, char *, char **, integer *, 
	    integer *, ftnlen);
    integer do_lio(f2c_state_t*, integer *, integer *, char *, ftnlen), 
	    e_wsle(f2c_state_t*), s_rnge(f2c_state_t*, char *, integer, char *
	    , integer);
    ...
    /* Module state */
    dafb2t_state_t* __state = get_dafb2t_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "DAFB2T", (ftnlen)6);
    }
...
/*     first record of the file. */
	...
	chkout_(__global_state, "DAFB2T", (ftnlen)6);
	return 0;
    }
...
L100001:
    if (iostat != 0) {
	setmsg_(__global_state, "Could not read ID word from file '#'. IOSTA"
		"T = #.", (ftnlen)49);
	errch_(__global_state, "#", binary, (ftnlen)1, binary_len);
...
L100002:
	...
	setmsg_(__global_state, "The attempt to write to file '#' failed. IO"
		"STAT = #.", (ftnlen)52);
    ...
    iostat = s_wsle(&__global_state->f2c, &__state->io___13);
    if (iostat != 0) {
	goto L100003;
    }
...
L100003:
    ...
    iostat = s_wsle(&__global_state->f2c, &__state->io___14);
    if (iostat != 0) {
	goto L100004;
    }
    iostat = do_lio(&__global_state->f2c, &__state->c__3, &__state->c__1, (
...
L100004:
    ...
    iostat = s_wsle(&__global_state->f2c, &__state->io___15);
    if (iostat != 0) {
	goto L100005;
    }
/* Writing concatenation */
...
/*     Write the arrays in forward order. */
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___23);
	if (iostat != 0) {
	    goto L100006;
	}
	iostat = do_lio(&__global_state->f2c, &__state->c__9, &__state->c__1, 
...
L100006:
	if (iostat != 0) {
	    dafcls_(__global_state, &handle);
	    setmsg_(__global_state, "The attempt to write to file '#' failed"
		    ". IOSTAT = #.", (ftnlen)52);
	    errfnm_(__global_state, "#", text, (ftnlen)1);
	    errint_(__global_state, "#", &iostat, (ftnlen)1);
	    sigerr_(__global_state, "SPICE(DAFWRITEFAIL)", (ftnlen)19);
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___24);
	if (iostat != 0) {
	    goto L100007;
	}
/* Writing concatenation */
...
L100007:
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___25);
	if (iostat != 0) {
	    goto L100008;
	}
	i__2 = nd;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    iostat = do_lio(&__global_state->f2c, &__state->c__5, &
		    __state->c__1, (char *)&dc[(i__3 = i__ - 1) < 125 && 0 <= 
		    i__3 ? i__3 : s_rnge(&__global_state->f2c, "dc", i__3, 
...
L100008:
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___27);
	if (iostat != 0) {
	    goto L100009;
	}
	i__3 = ni - 2;
	for (i__ = 1; i__ <= i__3; ++i__) {
	    iostat = do_lio(&__global_state->f2c, &__state->c__3, &
		    __state->c__1, (char *)&ic[(i__2 = i__ - 1) < 250 && 0 <= 
		    i__2 ? i__2 : s_rnge(&__global_state->f2c, "ic", i__2, 
...
L100009:
	...
	begin = ic[(i__2 = ni - 2) < 250 && 0 <= i__2 ? i__2 : s_rnge(&
		__global_state->f2c, "ic", i__2, "dafb2t_", (ftnlen)554)];
	end = ic[(i__2 = ni - 1) < 250 && 0 <= i__2 ? i__2 : s_rnge(&
		__global_state->f2c, "ic", i__2, "dafb2t_", (ftnlen)555)];
	while(begin <= end) {
/* Computing MIN */
	    i__2 = begin + 99;
	    chunk = min(i__2,end);
	    csize = chunk - begin + 1;
	    dafgda_(__global_state, &handle, &begin, &chunk, buffer);
	    if (failed_(__global_state)) {
		chkout_(__global_state, "DAFB2T", (ftnlen)6);
		return 0;
	    }
	    __state->io___33.ciunit = *text;
	    iostat = s_wsle(&__global_state->f2c, &__state->io___33);
	    if (iostat != 0) {
		goto L100010;
	    }
	    iostat = do_lio(&__global_state->f2c, &__state->c__3, &
		    __state->c__1, (char *)&csize, (ftnlen)sizeof(integer));
	    if (iostat != 0) {
		goto L100010;
	    }
	    iostat = e_wsle(&__global_state->f2c);
L100010:
	    if (iostat != 0) {
		dafcls_(__global_state, &handle);
		setmsg_(__global_state, "The attempt to write to file '#' fa"
			"iled. IOSTAT = #.", (ftnlen)52);
		errfnm_(__global_state, "#", text, (ftnlen)1);
		errint_(__global_state, "#", &iostat, (ftnlen)1);
		sigerr_(__global_state, "SPICE(DAFWRITEFAIL)", (ftnlen)19);
		chkout_(__global_state, "DAFB2T", (ftnlen)6);
	    ...
	    iostat = s_wsle(&__global_state->f2c, &__state->io___34);
	    if (iostat != 0) {
		goto L100011;
	    }
	    i__2 = csize;
	    for (i__ = 1; i__ <= i__2; ++i__) {
		iostat = do_lio(&__global_state->f2c, &__state->c__5, &
			__state->c__1, (char *)&buffer[(i__3 = i__ - 1) < 100 
			&& 0 <= i__3 ? i__3 : s_rnge(&__global_state->f2c, 
			"buffer", i__3, "dafb2t_", (ftnlen)586)], (ftnlen)
			sizeof(doublereal));
		if (iostat != 0) {
		    goto L100011;
		}
...
L100011:
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___35);
	if (iostat != 0) {
	    goto L100012;
	}
	iostat = do_lio(&__global_state->f2c, &__state->c__9, &__state->c__1, 
...
L100012:
	...
	iostat = s_wsle(&__global_state->f2c, &__state->io___36);
	if (iostat != 0) {
	    goto L100013;
	}
/* Writing concatenation */
...
/*     last: the internal file name brings up the rear. */
    ...
    iostat = s_wsle(&__global_state->f2c, &__state->io___37);
    if (iostat != 0) {
	goto L100014;
    }
    iostat = do_lio(&__global_state->f2c, &__state->c__9, &__state->c__1, 
...
L100014:
    ...
    iostat = s_wsle(&__global_state->f2c, &__state->io___38);
    if (iostat != 0) {
	goto L100015;
    }
/* Writing concatenation */

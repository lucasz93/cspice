...
extern zzspkap0_init_t __zzspkap0_init;
static inline zzspkap0_state_t* get_zzspkap0_state(cspice_t* state) {
	if (!state->zzspkap0)
		state->zzspkap0 = __cspice_allocate_module(sizeof(
	zzspkap0_state_t), &__zzspkap0_init, sizeof(__zzspkap0_init));
	return state->zzspkap0;

}

/* $Procedure ZZSPKAP0 ( S/P Kernel, apparent state ) */
/* Subroutine */ int zzspkap0_(cspice_t* __global_state, integer *targ, 
	...
	*starg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    zzspkap0_state_t* __state = get_zzspkap0_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZSPKAP0", (ftnlen)8);
    }
...
/*        Locate the flag in our list of flags. */
	...
	if (i__ == 0) {
	    ...
	    chkout_(__global_state, "ZZSPKAP0", (ftnlen)8);
	    return 0;
	}
...
/*     light time. */
    ...
    zzspksb0_(__global_state, targ, et, ref, starg, ref_len);
...
/*     the observer remains where he is. */
	...
	d__1 = *et + ltsign * *lt;
	zzspksb0_(__global_state, targ, &d__1, ref, starg, ref_len);
	vsubg_(__global_state, starg, sobs, &__state->c__6, tstate);
	moved_(__global_state, tstate, &__state->c__6, starg);
	*lt = vnorm_(__global_state, starg) / clight_(__global_state);
    }

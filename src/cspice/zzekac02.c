/* zzekac02.f -- translated by f2c (version 19991025).
...
extern zzekac02_init_t __zzekac02_init;
static inline zzekac02_state_t* get_zzekac02_state(cspice_t* state) {
	if (!state->zzekac02)
		state->zzekac02 = __cspice_allocate_module(sizeof(
	zzekac02_state_t), &__zzekac02_init, sizeof(__zzekac02_init));
	return state->zzekac02;

}

/* $Procedure     ZZEKAC02 ( EK, add class 2 column to segment ) */
/* Subroutine */ int zzekac02_(cspice_t* __global_state, integer *handle, 
	...
	integer *rcptrs, integer *wkindx)
{
    ...
    /* Module state */
    zzekac02_state_t* __state = get_zzekac02_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "ZZEKAC02", (ftnlen)8);
    }
...
/*     This column had better be class 2. */
	...
	setmsg_(__global_state, "Column class code # found in descriptor for"
		" column #.  Class should be 2.", (ftnlen)73);
	...
	chkout_(__global_state, "ZZEKAC02", (ftnlen)8);
	return 0;
    }
...
/*     items in the current page. */
	...
	if (nullok && nlflgs[from - 1]) {
	    adrbuf[(i__1 = bufptr - 1) < 126 && 0 <= i__1 ? i__1 : s_rnge(&
		    __global_state->f2c, "adrbuf", i__1, "zzekac02_", (ftnlen)
		    379)] = -2;
	} else {
	    adrbuf[(i__1 = bufptr - 1) < 126 && 0 <= i__1 ? i__1 : s_rnge(&
		    __global_state->f2c, "adrbuf", i__1, "zzekac02_", (ftnlen)
	    ...
	    page[(i__1 = to - 1) < 128 && 0 <= i__1 ? i__1 : s_rnge(&
		    __global_state->f2c, "page", i__1, "zzekac02_", (ftnlen)
		    384)] = dvals[from - 1];
	    ++to;
	    ++nwrite;
	    ++n;

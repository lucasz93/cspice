...
/* Subroutine */ int spkapp_(cspice_t* __global_state, integer *targ, 
	...
	*starg, doublereal *lt, ftnlen ref_len, ftnlen abcorr_len)
{
    ...
    /* Module state */
    spkapp_state_t* __state = get_spkapp_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "SPKAPP", (ftnlen)6);
    }
...
/*        Locate the flag in our list of flags. */
	...
	if (i__ == 0) {
	    ...
	    chkout_(__global_state, "SPKAPP", (ftnlen)6);
	    return 0;
	}
...
/*     light time. */
    ...
    spkssb_(__global_state, targ, et, ref, starg, ref_len);
...
/*     the observer remains where he is. */
	...
	d__1 = *et + ltsign * *lt;
	spkssb_(__global_state, targ, &d__1, ref, starg, ref_len);
	vsubg_(__global_state, starg, sobs, &__state->c__6, tstate);
	moved_(__global_state, tstate, &__state->c__6, starg);
	*lt = vnorm_(__global_state, starg) / clight_(__global_state);
    }

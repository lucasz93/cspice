...
static inline ckgr03_state_t* get_ckgr03_state(cspice_t* state) {
	if (!state->ckgr03)
...
/* Subroutine */ int ckgr03_(cspice_t* __global_state, integer *handle, 
	doublereal *descr, integer *recno, doublereal *record)
{
    ...
    /* Module state */
    ckgr03_state_t* __state = get_ckgr03_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
	...
	chkin_(__global_state, "CKGR03", (ftnlen)6);
    }
...
/*           big the pointing portion of the returned record will be. */
    ...
    if (icd[2] != 3) {
	setmsg_(__global_state, "Data type of the segment should be 3: Passe"
	...
	chkout_(__global_state, "CKGR03", (ftnlen)6);
	return 0;
    }
    if (icd[3] == 1) {
	psiz = 7;

...
/* Subroutine */ int gfsep_(cspice_t* __global_state, char *targ1, char *
	...
	nw, doublereal *work, doublereal *result, ftnlen targ1_len, ftnlen 
	shape1_len, ftnlen frame1_len, ftnlen targ2_len, ftnlen shape2_len, 
	ftnlen frame2_len, ftnlen abcorr_len, ftnlen obsrvr_len, ftnlen 
	relate_len)
{
    ...
    /* Module state */
    gfsep_state_t* __state = get_gfsep_state(__global_state);
/* $ Abstract */
...
/*     Standard SPICE error handling. */
    ...
    chkin_(__global_state, "GFSEP", (ftnlen)5);
	...
	chkout_(__global_state, "GFSEP", (ftnlen)5);
	return 0;
    }
    if (*nw < 5) {
    ...
    if (sized_(__global_state, result) < 2 || ! even_(__global_state, &i__1)) 
	    {
...
/*    Set the TARGET1 body-fixed frame name and shape model identifier. */
    ...
    s_copy(&__global_state->f2c, qpnams, "TARGET1", (ftnlen)80, (ftnlen)7);
    s_copy(&__global_state->f2c, qcpars, targ1, (ftnlen)80, targ1_len);
    s_copy(&__global_state->f2c, qpnams + 80, "FRAME1", (ftnlen)80, (ftnlen)6)
	    ;
    s_copy(&__global_state->f2c, qcpars + 80, frame1, (ftnlen)80, frame1_len);
    s_copy(&__global_state->f2c, qpnams + 160, "SHAPE1", (ftnlen)80, (ftnlen)
	    6);
    s_copy(&__global_state->f2c, qcpars + 160, shape1, (ftnlen)80, shape1_len)
	    ;

/*    Set the TARGET2 body-fixed frame name and shape model identifier. */

    s_copy(&__global_state->f2c, qpnams + 240, "TARGET2", (ftnlen)80, (ftnlen)
	    7);
    s_copy(&__global_state->f2c, qcpars + 240, targ2, (ftnlen)80, targ2_len);
    s_copy(&__global_state->f2c, qpnams + 320, "FRAME2", (ftnlen)80, (ftnlen)
	    6);
    s_copy(&__global_state->f2c, qcpars + 320, frame2, (ftnlen)80, frame2_len)
	    ;
    s_copy(&__global_state->f2c, qpnams + 400, "SHAPE2", (ftnlen)80, (ftnlen)
	    6);
    s_copy(&__global_state->f2c, qcpars + 400, shape2, (ftnlen)80, shape2_len)
	    ;

/*     Observer, aberration and calculation reference frame settings. */

    s_copy(&__global_state->f2c, qpnams + 480, "OBSERVER", (ftnlen)80, (
	    ftnlen)8);
    s_copy(&__global_state->f2c, qcpars + 480, obsrvr, (ftnlen)80, obsrvr_len)
	    ;
    s_copy(&__global_state->f2c, qpnams + 560, "ABCORR", (ftnlen)80, (ftnlen)
	    6);
    s_copy(&__global_state->f2c, qcpars + 560, abcorr, (ftnlen)80, abcorr_len)
	    ;
...
/*     Progress report and bail-out options are set to .FALSE. */
    ...
    gfevnt_(__global_state, (U_fp)gfstep_, (U_fp)gfrefn_, "ANGULAR SEPARATION"

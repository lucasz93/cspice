/* spcacc.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int spcacc_(integer *dafhdl, char *buffer, ftnlen buffer_len)
{
    /* Builtin functions */
...
/*     Store text from a character cell buffer in the comment area of a */
/*     binary DAF file, appending it to whatever text may already have */
/*     been stored there. */
...
/*          SPICE(NONPOSBUFLENGTH) will be signaled. */

/*     2)   If the end of of comment marker is not found, then the error */
/*          SPICE(MISSINGEOT) will be signaled. ( NOTE: the end comment */
...
/*     This routine does the same job as SPCACB except that it does not */
/*     add an extra blank line separating input comments from existing */
/*     comments. */
...
/*     in BUFFER will be appended to these comments. */
...
/* -    Beta Version 1.0.0, 21-JAN-2016 (KRG,BVS) */

/*        Based on SPCACB version 1.1.0, 18-MAY-2004 with a blank */
/*        separator line removed. */

/* -& */
/* $ Index_Entries */

/*      WRITE A CELL CHARACTER BUFFER TO A DAF COMMENT AREA */
